...
export default function Footer() {
  ...
  const handleNewsletterSubmit = (e: React.FormEvent) => {
    ...
    if (!newsletterEmail || !newsletterEmail.includes("@")) {
      ...
      setNewsletterMessage(tf.dispatchError);
      return;
    }
    ...
    setTimeout(() => {
      ...
      setNewsletterMessage(tf.dispatchSuccess);
      setNewsletterEmail("");
    }, 900);
  };
  ...
  return (
    <footer dir={isRTL ? "rtl" : "ltr"} className="relative w-full border-t border-white/10 bg-[#030307] pt-24 pb-12 overflow-hidden font-sans z-10">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 w-full flex flex-col items-center">
          ...
          <div className="relative z-10 grid items-center gap-10 lg:grid-cols-12">
            <div className="lg:col-span-7">
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/25 text-[11px] font-black uppercase tracking-widest text-emerald-400 mb-5 shadow-inner">
                <Sparkles size={13} className="animate-pulse" /> {tf.communityBannerBadge}
              </div>

              <h3 className="mb-4 text-2xl sm:text-3xl lg:text-4xl font-black text-white tracking-tight leading-tight">
                {tf.communityBannerTitle} <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-teal-300 to-emerald-500">{tf.communityBannerHighlight}</span>
              </h3>

              <p className="text-neutral-300 mb-6 text-sm sm:text-base leading-relaxed text-justify">
                {tf.communityBannerDesc}
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
                <div className="flex items-center gap-2.5 p-3 rounded-xl bg-white/[0.03] border border-white/5">
                  ...
                  <span className="text-xs font-semibold text-neutral-200">{tf.perk1}</span>
                </div>
                <div className="flex items-center gap-2.5 p-3 rounded-xl bg-white/[0.03] border border-white/5">
                  <CheckCircle2 size={16} className="text-emerald-400 shrink-0" />
                  <span className="text-xs font-semibold text-neutral-200">{tf.perk2}</span>
                </div>
                <div className="flex items-center gap-2.5 p-3 rounded-xl bg-white/[0.03] border border-white/5">
                  <CheckCircle2 size={16} className="text-emerald-400 shrink-0" />
                  <span className="text-xs font-semibold text-neutral-200">{tf.perk3}</span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                >
                  ...
                  <span>{tf.joinWhatsApp}</span>
                  <ArrowRight size={15} className={`transition-transform ${isRTL ? 'group-hover/btn:-translate-x-1 rotate-180' : 'group-hover/btn:translate-x-1'}`} />
                </a>
              ...
              <div className="relative group/img w-full max-w-sm">
                ...
                <div className="relative rounded-3xl overflow-hidden border border-white/15 bg-neutral-900 shadow-2xl">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 p-4 rounded-2xl bg-black/60 backdrop-blur-md border border-white/10 flex items-center justify-between">
                    <div>
                      <p className="text-white font-black text-sm">{tf.activeNations}</p>
                      <p className="text-neutral-400 text-xs">{tf.broadcastDesc}</p>
                    </div>
        ...
        <div className="w-full border-b border-white/10 pb-16 mb-16 overflow-hidden flex flex-col items-center">
          <div className="flex flex-col items-center justify-center mb-8 px-4 text-center">
            <span className="px-4 py-1.5 rounded-full bg-yellow-500/10 border border-yellow-500/20 text-[10px] font-black uppercase tracking-widest text-yellow-400 mb-3 shadow-inner flex items-center gap-1.5">
              <Award size={12} /> {tf.alliancesBadge}
            </span>
        ...
        <div className="flex flex-col items-center text-center mb-20 group cursor-default max-w-3xl">
          ...
          {/* Academy Name + AFGHANISTAN FLAG BADGE */}
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            <div className="px-4 py-2 rounded-full bg-white/[0.04] border border-white/10 backdrop-blur-md hover:border-yellow-500/30 transition-all shadow-sm flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-neutral-200">{tf.ukReg} <span className="text-yellow-400 font-mono">#17063286</span></span>
            </div>
            <div className="px-4 py-2 rounded-full bg-white/[0.04] border border-white/10 backdrop-blur-md hover:border-emerald-500/30 transition-all shadow-sm flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-neutral-200">{tf.hq} <span className="text-neutral-300">{tf.hqLondon}</span></span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10 mb-16 border-b border-white/10 pb-16">
          ...
          <div className="flex flex-col gap-4">
            <h3 className="text-white font-black text-sm uppercase tracking-widest mb-1 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-yellow-400"></span> {tf.colEcosystem}
            </h3>
            <a 
            ...
            <div className="text-neutral-400 hover:text-yellow-400 hover:translate-x-1.5 transition-all text-sm font-medium flex items-center justify-between cursor-default">
              ...
              <span className="text-[9px] uppercase tracking-wider font-mono text-neutral-500 bg-white/5 px-1.5 py-0.5 rounded">{tf.holding || "Holding"}</span>
            </div>
            <Link 
          ...
          <div className="flex flex-col gap-4">
            <h3 className="text-white font-black text-sm uppercase tracking-widest mb-1 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-emerald-400"></span> {tf.colFaculties}
            </h3>
            ...
            <Link href={`/${currentLocale}/scholarships`} className="text-neutral-400 hover:text-emerald-400 hover:translate-x-1.5 transition-all text-sm font-medium flex items-center gap-1.5">
              <span>{tfac.f5Title}</span>
              <span className="text-[9px] uppercase tracking-wider font-black text-emerald-400 bg-emerald-500/15 px-1.5 py-0.5 rounded">{tc.free}</span>
            </Link>
          ...
          <div className="flex flex-col gap-4">
            <h3 className="text-white font-black text-sm uppercase tracking-widest mb-1 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-cyan-400"></span> {tf.colGovernance}
            </h3>
            ...
            <Link href={`/${currentLocale}/instructor-application`} className="text-neutral-400 hover:text-cyan-400 hover:translate-x-1.5 transition-all text-sm font-medium flex items-center gap-1.5">
              <span>{dict.nav.instructorApplication}</span>
              <span className="text-[9px] uppercase tracking-wider font-black text-yellow-400 bg-yellow-500/15 px-1.5 py-0.5 rounded">{tf.hiring || "Hiring"}</span>
            </Link>
          ...
          <div className="flex flex-col gap-5 sm:col-span-2">
            <h3 className="text-white font-black text-sm uppercase tracking-widest mb-1 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-purple-400"></span> {tf.colDispatch}
            </h3>
            ...
            <form onSubmit={handleNewsletterSubmit} className="flex flex-col gap-2">
              <div className="relative flex items-center">
                <Mail size={15} className={`absolute ${isRTL ? 'right-3.5' : 'left-3.5'} text-neutral-500`} />
                <input
                  type="email"
                  ...
                  className={`w-full ${isRTL ? 'pr-10 pl-28 text-right' : 'pl-10 pr-28'} py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-white text-xs placeholder:text-neutral-500 focus:outline-none focus:border-yellow-400/60 focus:bg-white/[0.07] transition-all`}
                />
                <button
                  type="submit"
                  disabled={newsletterStatus === "loading" || newsletterStatus === "success"}
                  className={`absolute ${isRTL ? 'left-1' : 'right-1'} px-3.5 py-1.5 rounded-lg bg-gradient-to-r from-yellow-400 to-amber-500 hover:from-yellow-300 hover:to-amber-400 text-black font-black text-[11px] uppercase tracking-wider flex items-center gap-1.5 shadow-sm transition-all hover:scale-105 active:scale-95 disabled:opacity-50`}
                >
                  {newsletterStatus === "loading" ? (
                    <Loader2 size={13} className="animate-spin" />
                  ) : (
                    <>
                      <span>{tf.dispatchBtn}</span>
            ...
            <div className="mt-1 relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.05] to-transparent p-4 backdrop-blur-xl">
              <div className="flex items-center justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-black text-neutral-200 uppercase tracking-wider">{tf.campusNode}</span>
                </div>
              ...
              >
                <div className="flex items-center gap-2">
                  <Cpu size={13} />
                  <span>{tf.studentDeskBtn}</span>
                </div>
                <ArrowRight size={13} className={`transition-transform ${isRTL ? 'group-hover/desk:-translate-x-1 rotate-180' : 'group-hover/desk:translate-x-1'}`} />
              </Link>
            </div>
          </div>
        ...
        <div className="w-full flex flex-col lg:flex-row items-center justify-between gap-6 text-neutral-400 text-xs font-medium">
          <div className="flex flex-col sm:flex-row items-center gap-2 text-center sm:text-left">
            <p>© {currentYear} Safi Academy. {tf.holdingCompany}. {tc.rightsReserved}</p>
            <span className="hidden sm:inline text-neutral-600">•</span>
            <span className="text-neutral-500">{tf.companiesHouse}</span>
          </div>

          <div className="flex flex-wrap items-center justify-center gap-5 text-neutral-400">
            <Link href={`/${currentLocale}/privacy-policy`} className="hover:text-white transition-colors">{tf.privacyPolicy}</Link>
            <Link href={`/${currentLocale}/terms`} className="hover:text-white transition-colors">{tf.terms}</Link>
            <Link href={`/${currentLocale}/delete-account`} className="hover:text-rose-400 text-neutral-400 transition-colors">{tf.deleteAccount}</Link>
            <Link href={`/${currentLocale}/donate`} className="hover:text-white transition-colors">{tf.scholarshipEndowment}</Link>
            ...
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-white/[0.04] border border-white/10" title="Global Latency: 24ms">
              ...
              <span className="text-neutral-300 text-[11px]">{tf.systemsOperational}</span>
            </div>
            ...
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-yellow-500/10 border border-yellow-500/20" title="Autonomous Neural Mentorship Engine">
              ...
              <span className="text-yellow-400 font-bold text-[11px]">{tf.poweredBy}</span>
            </div>
          </div>
        </div>
