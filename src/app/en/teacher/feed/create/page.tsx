...
export default function TeacherCreatePostPage() {
  ...
  // دسته‌بندی‌های کاملاً حرفه‌ای و مناسب برای اساتید
  const moods = [
    "Announcement", 
    "Resource", 
    "Education", 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 4. بازگشت به فید اساتید
      router.push("/en/teacher/feed");
      router.refresh();
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-8 py-8 sm:py-12 font-sans relative min-h-screen">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-xs text-neutral-400 font-medium mt-1">Broadcast educational insights and announcements to your students and peers.</p>
        </div>
      </div>
        ...
        <div className="space-y-8 relative z-10">
              ...
              {moods.map((m) => (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 border ${
                    selectedMood === m
                      ? "bg-fuchsia-500/10 text-fuchsia-500 border-fuchsia-500/40 shadow-[0_0_15px_rgba(217,70,239,0.2)]"
                      : "bg-[#030305]/50 text-neutral-500 border-white/5 hover:border-white/20 hover:text-neutral-300"
                  }`}
                >
                  {selectedMood === m && <CheckCircle2 size={14} className="text-fuchsia-500" />}
                  {m}
                </button>
              ))}
            </div>
          ...
          <div className="space-y-3">
            ...
            <input
              ...
              className="w-full bg-[#030305]/60 border border-white/10 rounded-[1.5rem] px-6 py-5 text-white placeholder-neutral-600 font-bold focus:outline-none focus:border-fuchsia-500 focus:ring-1 focus:ring-fuchsia-500 transition-all shadow-inner text-lg"
              required
            />
          </div>
          ...
          <div className="space-y-3">
            ...
            <textarea
              placeholder="Elaborate your educational content, setup, or announcement here..."
              value={content}
              onChange={(e) => setContent(e.target.value)}
              rows={8}
              className="w-full bg-[#030305]/60 border border-white/10 rounded-[1.5rem] px-6 py-5 text-white placeholder-neutral-600 focus:outline-none focus:border-fuchsia-500 focus:ring-1 focus:ring-fuchsia-500 transition-all shadow-inner resize-none text-sm leading-relaxed"
              required
            />
          </div>
          ...
          <div className="space-y-3">
            ...
            {!imagePreview ? (
              <div 
                ...
                className="w-full border-2 border-dashed border-white/10 hover:border-fuchsia-500/50 rounded-[1.5rem] p-10 flex flex-col items-center justify-center gap-4 bg-[#030305]/40 hover:bg-fuchsia-500/5 transition-all cursor-pointer group"
              >
                <div className="w-16 h-16 rounded-full bg-white/5 group-hover:bg-fuchsia-500/10 flex items-center justify-center transition-colors">
                  <ImagePlus size={28} className="text-neutral-500 group-hover:text-fuchsia-500 transition-colors" />
                </div>
          ...
          <div className="flex items-center justify-end pt-2">
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-3 px-10 py-4 bg-gradient-to-r from-fuchsia-600 to-purple-600 text-white font-black text-xs uppercase tracking-widest rounded-xl hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:hover:scale-100 shadow-[0_10px_25px_rgba(217,70,239,0.4)]"
            >
              {isSubmitting ? (
                <>
