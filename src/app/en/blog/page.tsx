...
import { Calendar, User, ArrowRight, BookOpen, Clock, Tag, Search, Globe, X } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#050508] text-white font-sans pb-40 overflow-hidden" dir="ltr">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-12 pt-40">
        ...
        <section className="text-center max-w-4xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-yellow-500/10 border border-yellow-500/20 text-yellow-500 text-[10px] sm:text-xs font-black uppercase tracking-widest mb-8 shadow-inner">
          ...
          <p className="text-neutral-400 text-sm sm:text-lg max-w-3xl mx-auto leading-relaxed">
          ...
          <div className="max-w-2xl mx-auto mt-12 relative group px-2 sm:px-0">
            ...
            <div className="relative flex items-center bg-[#0a0a0f]/80 backdrop-blur-xl border border-white/10 rounded-full p-2 focus-within:border-yellow-500/50 transition-colors shadow-2xl">
              <Search className="text-yellow-500 ml-4 mr-2 shrink-0" size={24} />
              <input 
                type="text" 
                placeholder="Search articles..." 
                className="bg-transparent border-none outline-none w-full text-white placeholder:text-neutral-600 px-2 py-3 font-medium text-sm sm:text-base"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
              {searchTerm && (
                <button onClick={() => setSearchTerm("")} className="mr-3 text-neutral-600 hover:text-yellow-500"><X size={20} /></button>
              )}
            </div>
          </div>
        </section>
        ...
        <section className="mb-16 px-2 max-w-5xl mx-auto flex justify-center">
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 bg-white/5 p-2 rounded-full border border-white/10 backdrop-blur-md min-w-max shadow-inner overflow-x-auto pb-2">
            {categories.map((cat) => (
              <button
                key={cat}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-24 bg-[#0a0a0e] border border-white/5 rounded-[3rem] flex flex-col items-center gap-4">
            <Globe className="w-16 h-16 text-neutral-700" />
            <h3 className="text-xl font-bold text-neutral-400">No articles available yet.</h3>
          </div>
        ) : (
                ...
                return (
                  ...
                  >
                    <div className="relative h-56 w-full overflow-hidden bg-neutral-900">
                      ...
                      <span className="absolute bottom-4 right-4 z-20 bg-black/70 backdrop-blur-sm text-yellow-500 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg flex items-center gap-1.5">
                        <Clock size={12} /> {readTime} min read
                      </span>
                    </div>

                    <div className="p-6 sm:p-8 flex flex-col flex-1 relative z-20">
                      <div className="flex items-center justify-between gap-4 text-neutral-500 text-[11px] font-bold tracking-wide mb-5">
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          <span className="bg-yellow-500 text-black text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md">{post.category || "General"}</span>
                        </div>
                      </div>
                      ...
                      {/* هدايت مستقیم به آدرس پوشه ثابت انگلیسی */}
                      <Link
                        href={`/en/blog/${post.slug}`}
                        className="mt-auto flex items-center gap-2 text-xs font-black uppercase tracking-widest text-neutral-400 group-hover:text-white transition-all pt-5 border-t border-white/5 w-full"
                      >
                        Read Full Insights
