...
import { 
  ...
  Trophy, Megaphone, Video, Headset, Settings, Grid, Rss, SquarePen, User
} from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const isSocialRoute = pathname.includes('/admin/feed');
  ...
  const myProfilePath = userProfile?.id ? `/en/admin/feed/profile/${userProfile.id}` : "/en/admin/feed/profile";

  const menuItems = [
    { name: "Overview", path: "/en/admin", icon: <LayoutDashboard size={22} />, disabled: false },
    { name: "Academy Feed", path: "/en/admin/feed", icon: <Rss size={22} />, disabled: false },
    { name: "Manage Network", path: "/en/admin/feed/network", icon: <Users size={22} />, disabled: false },
    { name: "Create Post", path: "/en/admin/feed/create", icon: <SquarePen size={22} />, disabled: false },
        ...
        <nav className="flex-1 overflow-y-auto space-y-2 custom-scrollbar pr-1">
          {menuItems.map((item) => {
            ...
            const isActive = item.name === "My Profile" 
                ? pathname.includes("/admin/feed/profile")
                : item.path === "/en/admin" 
                    ? pathname === "/en/admin" 
      ...
      {!isFullScreenRoute && (
        <div className="lg:hidden fixed bottom-6 left-4 right-4 h-[75px] bg-[#09090e]/95 backdrop-blur-3xl border border-white/10 z-50 px-2 rounded-[2.5rem] flex justify-between items-center shadow-[0_20px_50px_rgba(0,0,0,0.9),0_0_20px_rgba(244,63,94,0.05)]">
          {isSocialRoute ? (
            <>
              {/* FEED NAV */}
              <Link href="/en/admin/feed" className="relative flex flex-col items-center justify-center w-[20%] h-full group">
                {pathname === "/en/admin/feed" && <div className="absolute inset-0 bg-gradient-to-t from-rose-500/20 to-transparent rounded-[2rem] opacity-100"></div>}
                <Rss size={22} className={`z-10 transition-all ${pathname === "/en/admin/feed" ? "-translate-y-2 text-rose-400" : "text-neutral-500 group-hover:text-neutral-300"}`} />
                <span className={`absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all z-10 ${pathname === "/en/admin/feed" ? "text-rose-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2"}`}>Feed</span>
              </Link>
              {/* NETWORK NAV */}
              <Link href="/en/admin/feed/network" className="relative flex flex-col items-center justify-center w-[20%] h-full group">
                {pathname.includes("/admin/feed/network") && <div className="absolute inset-0 bg-gradient-to-t from-indigo-500/20 to-transparent rounded-[2rem] opacity-100"></div>}
                <Users size={22} className={`z-10 transition-all ${pathname.includes("/admin/feed/network") ? "-translate-y-2 text-indigo-400" : "text-neutral-500 group-hover:text-neutral-300"}`} />
                <span className={`absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all z-10 ${pathname.includes("/admin/feed/network") ? "text-indigo-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2"}`}>Network</span>
              </Link>
              {/* CREATE POST (FAB CENTER) */}
              <Link href="/en/admin/feed/create" className="relative flex flex-col items-center justify-start w-[20%] h-full group -mt-8">
                <div className="absolute top-1 w-16 h-16 bg-rose-500/40 blur-[12px] rounded-full group-hover:bg-rose-500/60 transition-all duration-300"></div>
                <div className="relative z-10 w-[56px] h-[56px] bg-gradient-to-br from-rose-500 to-purple-600 rounded-full flex items-center justify-center border-[5px] border-[#09090e] shadow-xl group-hover:scale-105 transition-transform duration-300">
                  <SquarePen size={22} className="text-white ml-0.5" strokeWidth={2.5} />
                </div>
                <span className="text-[9px] font-black tracking-widest uppercase text-rose-500 mt-1.5 drop-shadow-[0_0_8px_#f43f5e]">Post</span>
              </Link>
              {/* PROFILE NAV */}
              <Link href={myProfilePath} className="relative flex flex-col items-center justify-center w-[20%] h-full group">
                {pathname.includes("/admin/feed/profile") && <div className="absolute inset-0 bg-gradient-to-t from-blue-500/20 to-transparent rounded-[2rem] opacity-100"></div>}
                <User size={22} className={`z-10 transition-all ${pathname.includes("/admin/feed/profile") ? "-translate-y-2 text-blue-400" : "text-neutral-500 group-hover:text-neutral-300"}`} />
                <span className={`absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all z-10 ${pathname.includes("/admin/feed/profile") ? "text-blue-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2"}`}>Profile</span>
              </Link>
              {/* MENU NAV */}
            ...
            <>
              ...
              {/* FEED ENTRY NAV */}
              <Link href="/en/admin/feed" className="relative flex flex-col items-center justify-center w-[25%] h-full group">
