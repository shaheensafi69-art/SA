...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // 🔥 تشخیص هوشمند صفحات فول‌اسکرین ادمین (مثل ورود به کلاس زنده به عنوان ناظر)
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      if (user && !userError) {
        ...
        if (profile) {
          // محافظت امنیتی: فقط ادمین‌ها حق ورود دارند
  ...
  // 🚀 لیست کامل منوهای یک LMS قدرتمند برای ادمین
  const menuItems = [
  ...
  // رنگ‌بندی اختصاصی هر بخش برای هدایت چشمی سریع ادمین
  ...
  if (!isReady) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#020202] text-white">
         <div className="w-16 h-16 border-4 border-rose-500 border-t-transparent rounded-full animate-spin mb-4 shadow-[0_0_20px_rgba(244,63,94,0.3)]"></div>
         <span className="text-rose-500 font-bold uppercase tracking-widest text-sm animate-pulse">Loading Command Center...</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-[#020202] text-white font-sans overflow-hidden relative">
      ...
      {/* 🔴 هاله‌های نوری اختصاصی پنل ادمین (رنگ‌های قدرت: زرشکی و بنفش تیره) */}
      <div className="absolute top-[-10%] left-[-10%] w-[40vw] h-[40vw] bg-rose-600/10 rounded-full blur-[120px] pointer-events-none z-0"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[30vw] h-[30vw] bg-purple-700/5 rounded-full blur-[100px] pointer-events-none z-0"></div>
      ...
      <aside className="hidden lg:flex w-[280px] bg-[#050505]/80 backdrop-blur-3xl border-r border-white/5 flex-col relative z-20 shrink-0">
        <div className="h-24 px-8 flex items-center gap-4 border-b border-white/5 shrink-0">
           <Link href="/en/admin" className="flex items-center gap-4 group">
             <div className="relative flex items-center justify-center group-hover:scale-105 transition-transform">
               <div className="absolute inset-0 bg-rose-500/20 blur-[15px] rounded-full scale-150"></div>
               <img src="/logo-without-b.png" alt="Safi Academy" className="relative z-10 w-12 h-12 object-contain drop-shadow-[0_0_10px_rgba(244,63,94,0.5)]" />
             </div>
             <div>
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-4 px-4 py-3.5 rounded-2xl font-bold text-sm transition-all duration-300 relative overflow-hidden group ${
                  isActive ? "text-white shadow-[0_10px_20px_rgba(244,63,94,0.15)] scale-[1.02]" : "text-neutral-400 hover:bg-white/5 hover:text-white"
                }`}
              >
                {isActive && <div className="absolute inset-0 bg-gradient-to-r from-rose-600 to-purple-800 opacity-100"></div>}
                <span className={`relative z-10 text-xl transition-transform ${isActive ? "" : "group-hover:scale-110"}`}>{item.icon}</span>
                <span className="relative z-10 tracking-wide">{item.name}</span>
                {isActive && item.name === "Live Studio" && <span className="absolute right-4 w-2 h-2 rounded-full bg-red-400 animate-pulse border border-white/50 z-10"></span>}
              </Link>
            );
          })}
        </nav>
        ...
        <div className="p-5 border-t border-white/5 bg-black/40 shrink-0">
          <div className="bg-white/5 border border-white/10 p-3 rounded-2xl flex items-center justify-between hover:bg-white/10 transition-colors">
            <div className="flex items-center gap-3 overflow-hidden">
              <div className="w-10 h-10 rounded-xl border border-rose-500/30 overflow-hidden shrink-0 bg-neutral-900 flex items-center justify-center">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-rose-400 font-black tracking-widest uppercase mt-0.5">Super Admin</p>
              </div>
            </div>
          </div>
          <button onClick={handleLogout} className="mt-3 flex items-center justify-center gap-2 px-4 py-3 text-neutral-400 hover:text-red-400 bg-white/5 hover:bg-red-500/10 rounded-xl text-xs font-bold transition-all w-full border border-white/5 hover:border-red-500/20">
             Secure Sign Out
          </button>
        </div>
      </aside>
      ...
      <div className="hidden lg:flex absolute top-6 right-10 z-50">
         <button className="w-12 h-12 bg-neutral-900/60 backdrop-blur-md border border-white/10 rounded-2xl flex items-center justify-center text-neutral-400 hover:text-rose-400 hover:border-rose-500/30 transition-all relative group shadow-lg">
           ...
           <span className="absolute top-3 right-3 w-2 h-2 bg-rose-500 rounded-full animate-pulse shadow-[0_0_5px_rgba(244,63,94,1)]"></span>
         </button>
      </div>
      ...
      {!isFullScreenRoute && (
        <div className="lg:hidden fixed top-0 left-0 right-0 h-16 flex items-center justify-between px-5 bg-[#020202]/80 backdrop-blur-2xl border-b border-white/5 z-40">
          <div className="flex items-center gap-3 relative">
            ...
            <img src="/logo-without-b.png" alt="Safi Academy" className="relative z-10 w-9 h-9 object-contain filter drop-shadow-[0_0_8px_rgba(244,63,94,0.5)]" />
            <span className="relative z-10 font-black text-xs tracking-widest text-white uppercase">Safi Academy</span>
          </div>
          
          <button className="w-10 h-10 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-neutral-400 hover:text-rose-400 transition-colors relative">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
              <span className="absolute top-2.5 right-2.5 w-1.5 h-1.5 bg-rose-500 rounded-full animate-pulse shadow-[0_0_5px_rgba(244,63,94,1)]"></span>
          </button>
        </div>
      )}
      ...
      {!isFullScreenRoute && (
        <div className="lg:hidden fixed bottom-6 left-6 right-6 h-[72px] bg-neutral-950/80 backdrop-blur-3xl border border-white/10 z-50 px-3 rounded-full flex justify-around items-center shadow-[0_20px_50px_rgba(0,0,0,0.9),0_0_20px_rgba(244,63,94,0.05)]">
          {[
            { name: "Overview", path: "/en/admin", icon: "📊" },
            { name: "Students", path: "/en/admin/manage-students", icon: "👨‍🎓" },
            { name: "Finance", path: "/en/admin/finance", icon: "💰" },
            ...
            return (
              <Link key={item.path} href={item.path} className="relative flex flex-col items-center justify-center w-16 h-full group">
                {isActive && <div className="absolute inset-0 bg-gradient-to-t from-rose-500/15 to-transparent rounded-full opacity-100 transition-opacity duration-300"></div>}
                <span className={`text-xl transition-all duration-500 ease-out z-10 ${isActive ? "-translate-y-2 scale-110 drop-shadow-[0_0_8px_rgba(244,63,94,0.5)]" : "text-neutral-500 group-hover:-translate-y-1 group-hover:text-neutral-300"}`}>{item.icon}</span>
                <span className={`absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all duration-500 z-10 ${isActive ? "text-rose-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0"}`}>{item.name}</span>
                {isActive && <div className="absolute bottom-1 w-1 h-1 rounded-full bg-rose-500 shadow-[0_0_5px_#f43f5e]"></div>}
              </Link>
            );
          })}
          <button onClick={() => setIsMobileMenuOpen(true)} className="relative flex flex-col items-center justify-center w-16 h-full group">
            ...
            <span className="absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all duration-500 z-10 text-neutral-500 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0">Menu</span>
          </button>
        </div>
      )}
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-[#020202]/95 backdrop-blur-3xl z-[100] flex flex-col animate-[fadeIn_0.2s_ease-out] lg:hidden">
          <div className="h-16 px-5 border-b border-white/5 flex justify-between items-center bg-black/40 shrink-0">
            <span className="font-black text-white uppercase tracking-widest text-xs">Command Center Menu</span>
            ...
            <div className="mt-8 pt-6 border-t border-white/10">
               {userProfile && (
                <div className="flex items-center gap-4 mb-6 bg-white/[0.02] border border-white/5 p-4 rounded-3xl">
                  <img src={userProfile.avatar_url || "https://i.pravatar.cc/150"} alt="User" className="w-12 h-12 rounded-xl border border-rose-500 object-cover" />
