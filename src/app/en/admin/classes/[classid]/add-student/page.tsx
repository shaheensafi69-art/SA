...
import { 
  Loader2, ArrowLeft, Search, Mail, Phone, 
  CalendarDays, CheckCircle2, ShieldAlert, 
  BadgeCent, Users, UserPlus 
} from "lucide-react";
...
export default function AdminAddStudentToClassPage() {
  ...
  return (
    <div className="min-h-screen bg-[#030305] text-white p-4 sm:p-6 md:p-10 relative overflow-hidden pb-40" dir="ltr">
                  ...
                  <div 
                    ...
                    className={`relative overflow-hidden cursor-pointer bg-[#0a0a0f]/90 border rounded-[2rem] p-6 transition-all duration-300 shadow-xl backdrop-blur-md flex flex-col gap-5 ${
                      selectedStudent?.id === user.id 
                        ? "border-rose-500 shadow-[0_0_30px_rgba(244,63,94,0.15)] ring-1 ring-rose-500 scale-[1.02]" 
                    ...
                    <div className="flex items-center gap-4 relative z-10 border-b border-white/5 pb-4">
                      <div className="w-14 h-14 rounded-2xl overflow-hidden bg-neutral-900 border border-white/10 shrink-0 flex items-center justify-center shadow-inner">
                        {user.avatar_url ? (
                          ...
                          <span className="text-xl font-black text-rose-500">{user.first_name.charAt(0)}</span>
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="text-base font-black text-white truncate">{user.first_name} {user.last_name}</h3>
                        {user.father_name && <p className="text-[10px] font-bold uppercase tracking-widest text-neutral-500 truncate mt-0.5">F/O: {user.father_name}</p>}
                      </div>
      ...
      {selectedStudent && (
        <div className="fixed bottom-0 left-0 right-0 z-50 p-4 sm:p-6 bg-gradient-to-t from-[#030305] via-[#0a0a0f]/95 to-transparent backdrop-blur-md animate-[slideInUp_0.3s_ease-out]">
          <div className="max-w-4xl mx-auto bg-rose-950/20 border border-rose-500/30 rounded-[2rem] p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-[0_-10px_40px_rgba(244,63,94,0.15)] backdrop-blur-2xl">
            
            <div className="flex items-center gap-4 flex-1 w-full sm:w-auto">
              <div className="w-12 h-12 rounded-xl bg-black border border-rose-500/30 flex items-center justify-center shrink-0">
                {selectedStudent.avatar_url ? (
                  <img src={selectedStudent.avatar_url} className="w-full h-full object-cover rounded-xl" />
                ) : (
                  <span className="text-rose-500 font-black">{selectedStudent.first_name.charAt(0)}</span>
                )}
              </div>
