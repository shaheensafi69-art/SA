...
export default function AdminFeedPage() {
  ...
  const toggleLike = async (post: PostItem) => {
    ...
    try {
      ...
      fetchFeedAndUsers(); // بازگشت در صورت خطا
    }
  };

  const deletePost = async (postId: string) => {
    if (!window.confirm("Admin Action: Are you sure you want to delete this post permanently?")) return;
    try {
  ...
  if (isLoading) {
    return (
      <div className="w-full h-[80vh] flex flex-col items-center justify-center bg-transparent gap-4">
        ...
        <span className="text-xs font-black text-rose-500 uppercase tracking-widest animate-pulse">Loading Feed...</span>
      </div>
    );
  }
        ...
        <div className="hidden lg:block lg:col-span-4 xl:col-span-3 space-y-6 sticky top-8 z-10 xl:-ml-4">
          <div className="bg-[#0a0a0f]/90 border border-white/5 p-6 rounded-[2.5rem] backdrop-blur-2xl shadow-[0_20px_40px_rgba(0,0,0,0.4)] relative overflow-hidden group">
            ...
            <div className="flex items-center gap-4 mb-6 relative z-10">
              <div className="p-3 bg-gradient-to-br from-rose-500 to-purple-600 rounded-[1.2rem] text-white shadow-lg shadow-rose-500/20">
                <ShieldCheck className="w-6 h-6" />
              </div>
              <div>
                <h2 className="text-white font-black text-sm tracking-wide">Network Watch</h2>
                <p className="text-[10px] text-neutral-400 font-bold uppercase tracking-widest mt-0.5">Admin Overview</p>
              </div>
            </div>
            ...
            <div className="space-y-3 relative z-10">
              {exploreUsers.map((user) => (
                <Link
                  key={user.id}
                  href={`/en/admin/feed/profile/${user.id}`}
                  className="flex items-center gap-4 p-3 rounded-[1.2rem] bg-white/[0.02] border border-white/5 hover:border-rose-500/30 hover:bg-white/[0.04] hover:shadow-[0_4px_15px_rgba(244,63,94,0.1)] transition-all duration-300 group/item"
                >
                  <div className="w-11 h-11 rounded-[1rem] bg-neutral-800 overflow-hidden border border-rose-500/20 flex items-center justify-center shrink-0 group-hover/item:border-rose-500 transition-colors">
                    {user.avatar_url ? (
                      <img src={user.avatar_url} alt="Avatar" className="w-full h-full object-cover" />
                    ) : (
                      <span className="text-rose-500 font-black text-sm">{user.first_name?.[0] || 'U'}</span>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-white font-black text-xs truncate group-hover/item:text-rose-300 transition-colors">
        ...
        <div className="col-span-1 lg:col-span-8 xl:col-span-9 space-y-6 w-full max-w-3xl mx-auto xl:max-w-4xl">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 bg-[#0a0a0f]/80 border border-white/5 p-5 sm:p-8 rounded-[2rem] backdrop-blur-xl shadow-2xl">
            <div>
              ...
              <p className="text-xs text-neutral-400 font-medium mt-1.5">Monitor all discussions, share announcements, and engage.</p>
            </div>
            <Link
              href="/en/admin/feed/create"
              className="hidden sm:flex px-6 py-4 bg-gradient-to-r from-rose-500 to-purple-600 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:scale-105 transition-transform duration-300 shadow-[0_10px_25px_rgba(244,63,94,0.4)] items-center justify-center gap-2"
            >
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-[#0a0a0f]/60 border border-white/5 rounded-[1.5rem] px-5 py-4 pl-14 text-white placeholder-neutral-500 font-medium focus:outline-none focus:border-rose-500 transition-colors shadow-lg"
            />
                    ...
                    <div className="flex items-center gap-3 sm:gap-4">
                      <Link href={`/en/admin/feed/profile/${post.studentId}`} className="w-10 h-10 sm:w-14 sm:h-14 rounded-[1.2rem] bg-neutral-800 border-2 border-rose-500/30 overflow-hidden flex items-center justify-center hover:scale-105 hover:border-rose-500 transition-all shrink-0">
                        {post.authorAvatar ? (
                          ...
                          <span className="text-rose-500 font-black text-lg">{post.authorName.charAt(0)}</span>
                        )}
                      </Link>
                      <div>
                        <Link href={`/en/admin/feed/profile/${post.studentId}`} className="text-white font-black text-xs sm:text-[15px] hover:text-rose-400 transition-colors tracking-wide">
                          {post.authorName}
                        </Link>
                    ...
                    <div className="flex items-center justify-between pt-4 pb-1 text-[11px] font-bold text-neutral-400">
                      {post.likesCount > 0 ? (
                        <div className="flex items-center gap-2">
                          <div className="p-1.5 bg-rose-500 rounded-full text-white shadow-[0_0_10px_rgba(244,63,94,0.5)]">
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-[1rem] transition-all font-black text-xs ${
                        post.isLikedByMe 
                        ? "text-rose-500 bg-rose-500/10 border border-rose-500/30 shadow-[0_0_15px_rgba(244,63,94,0.15)]" 
      ...
      elements.push(
        <div key={c.id} style={{ marginLeft: `${depth * 24}px` }} className="mb-4">
          <div className="flex gap-3 items-start">
            <div className="w-8 h-8 rounded-xl bg-neutral-800 shrink-0 overflow-hidden border border-rose-500/20 flex items-center justify-center">
              {avatarUrl ? <img src={avatarUrl} className="w-full h-full object-cover" alt="" /> : <span className="text-rose-500 text-xs font-black">{authorName.charAt(0)}</span>}
            </div>
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mt-1.5 ml-2">
                ...
                <button 
                  ...
                  className="text-[10px] font-black text-rose-500 hover:text-rose-400 transition-colors tracking-wide uppercase"
                >
                  Reply
                </button>
              </div>
    ...
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 pb-[100px] sm:p-4 sm:pb-4 bg-black/80 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
      <div className="bg-[#0a0a0f] border border-white/10 rounded-t-[2rem] sm:rounded-[2rem] w-full max-w-2xl max-h-[80vh] sm:max-h-[90vh] flex flex-col shadow-[0_30px_60px_rgba(0,0,0,0.8)] overflow-hidden animate-[slideUp_0.3s_ease-out]">
        ...
        <div className="p-5 sm:p-6 border-t border-white/5 bg-[#0a0a0f]/95 backdrop-blur-xl">
          {replyingToName && (
            <div className="flex items-center justify-between mb-3 px-3 py-2 bg-rose-500/10 rounded-xl border border-rose-500/20">
          ...
          <div className="flex items-end gap-3 pb-2 sm:pb-0">
            <textarea
              ...
              rows={1}
              className="flex-1 bg-neutral-900 border border-white/10 rounded-2xl px-5 py-4 text-white text-sm focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 resize-none min-h-[55px] max-h-[120px] transition-all"
            />
            <button
              onClick={handleSend}
