...
import { 
  ...
  Award, Flame, Wallet, Calendar, Share2, Link as LinkIcon, Activity, Loader2
} from "lucide-react";
...
export default function AdminUserProfilePage({ params }: { params: { id: string } }) {
  ...
  const deletePost = async (postId: string) => {
    if (!window.confirm("Admin Action: Are you sure you want to permanently delete this post?")) return;
    try {
  ...
  if (isLoading) {
    return (
      <div className="w-full min-h-[80vh] flex flex-col items-center justify-center bg-transparent gap-4">
        ...
        <span className="text-xs font-black text-rose-500 uppercase tracking-widest animate-pulse">Loading Profile...</span>
      </div>
    );
  }
  ...
  if (!profileData) {
    return (
      <div className="w-full h-[80vh] flex flex-col items-center justify-center">
        ...
        <Link href="/en/admin/feed" className="mt-4 text-rose-500 hover:underline font-bold">Return to Feed</Link>
      </div>
    );
  }
    ...
    <div className="w-full max-w-[85rem] mx-auto pb-24 pt-4 sm:pt-0 font-sans relative min-h-screen bg-[#030305]">
      ...
      <div className="hidden sm:flex items-center gap-4 px-8 py-6">
        ...
        <div>
          ...
          <p className="text-xs text-rose-500 font-bold mt-1 uppercase tracking-widest">{profileData.role.replace('_', ' ')}</p>
        </div>
      </div>
      ...
      <div className="sm:px-8">
        <div className="bg-[#0a0a0f] sm:bg-[#0a0a0f]/80 sm:border border-white/10 sm:rounded-[2.5rem] backdrop-blur-xl sm:shadow-2xl relative overflow-hidden mb-6 sm:mb-8 pb-6 sm:pb-8">
          ...
          {/* Cover */}
          <div className="h-32 sm:h-48 w-full bg-gradient-to-r from-[#0a0a0f] via-rose-500/30 to-purple-900/20 relative">
          ...
          <div className="px-5 sm:px-10 relative z-10 -mt-12 sm:-mt-20">
            <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 sm:gap-6 text-center sm:text-left">
              ...
              <div className="relative">
                <div className="w-24 h-24 sm:w-40 sm:h-40 rounded-[1.8rem] sm:rounded-[2rem] bg-[#050508] border-4 sm:border-[5px] border-[#0a0a0f] overflow-hidden flex items-center justify-center shadow-[0_0_20px_rgba(244,63,94,0.3)]">
                  {profileData.avatar_url ? (
                    <img src={profileData.avatar_url} alt="Avatar" className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-rose-500 font-black text-4xl sm:text-5xl">{profileData.first_name?.[0] || 'U'}</span>
                  )}
                </div>
                {isFaculty && (
              ...
              <div className="flex-1 w-full mt-2 sm:mt-0 sm:pb-2">
                ...
                <div className="flex items-center justify-center sm:justify-start gap-2 sm:gap-6 mt-2 w-full">
                  ...
                  {!isFaculty && (
                    <div className="flex-1 sm:flex-none flex flex-col items-center sm:flex-row sm:gap-2 bg-rose-500/10 sm:bg-transparent border sm:border-0 border-rose-500/20 py-2 sm:py-0 rounded-xl">
                      <span className="text-lg sm:text-xl font-black text-rose-500">{profileData.total_score || 0}</span>
                      <span className="text-[9px] sm:text-[10px] font-bold text-rose-500/70 uppercase tracking-widest">Score</span>
                    </div>
                  )}
                  <div className="flex-1 sm:flex-none flex flex-col items-center sm:flex-row sm:gap-2 bg-white/5 sm:bg-transparent border sm:border-0 border-white/5 py-2 sm:py-0 rounded-xl">
                ...
                <div className="w-full sm:w-auto mt-4 sm:mt-0 sm:pb-2">
                  <button
                    ...
                    className={`px-8 py-3.5 sm:py-4 flex items-center justify-center gap-2 rounded-2xl text-xs font-black uppercase tracking-widest transition-all duration-300 w-full sm:w-auto shadow-lg ${
                      ...
                      "bg-rose-500 text-white border border-rose-500 shadow-[0_0_20px_rgba(244,63,94,0.4)] hover:bg-rose-600"
                    }`}
                  >
          ...
          <div className="lg:col-span-4 xl:col-span-3 space-y-6 lg:sticky lg:top-8">
            <div className="bg-[#0a0a0f]/80 sm:border border-white/10 p-5 sm:p-8 rounded-3xl sm:rounded-[2.5rem] backdrop-blur-md sm:shadow-xl">
              <h3 className="text-xs sm:text-sm font-black text-white uppercase tracking-widest mb-4 flex items-center gap-2">
                <UserCheck size={16} className="text-rose-500"/> Biography
              </h3>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-1 gap-3">
                ...
                {/* Admin Access: Can see all private info */}
              ...
              {[
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-[11px] sm:text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap flex-1 sm:flex-none ${
                    activeTab === tab.id 
                    ? "bg-rose-500 text-white shadow-[0_0_15px_rgba(244,63,94,0.3)] border border-rose-500" 
                      ...
                      <div key={post.id} className="bg-[#0a0a0f]/80 sm:border border-white/10 rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-8 backdrop-blur-md space-y-4 sm:space-y-5 shadow-lg sm:shadow-[0_15px_35px_rgba(0,0,0,0.5)] transition-all">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3 sm:gap-4">
                            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full sm:rounded-[1rem] bg-neutral-800 border-2 border-rose-500/30 overflow-hidden flex items-center justify-center shrink-0">
                              {profileData.avatar_url ? (
                                <img src={profileData.avatar_url} alt="Avatar" className="w-full h-full object-cover" />
                              ) : (
                                <span className="text-rose-500 font-black">{profileData.first_name?.[0] || 'U'}</span>
                              )}
                            </div>
                            <div>
                          ...
                          <button onClick={() => deletePost(post.id)} className="text-neutral-500 hover:text-red-400 p-2 sm:p-2.5 bg-white/5 rounded-xl hover:bg-red-500/10 transition-colors" title="Delete as Admin">
                            <Trash2 size={16} />
                          </button>
                        </div>

                        {/* Mood */}
                        <div className="inline-block px-3 py-1 bg-rose-500/10 border-l-2 border-rose-500 rounded-r-lg text-rose-300 text-[9px] sm:text-[10px] font-black uppercase tracking-widest shadow-sm">
                          {post.moodTag}
                        </div>
                          ...
                          <div className="flex items-center justify-between pt-3 text-[10px] sm:text-[11px] font-bold text-neutral-400">
                            {post.likesCount > 0 ? (
                              <div className="flex items-center gap-1.5">
                                <div className="p-1 sm:p-1.5 bg-rose-500 rounded-full text-white shadow-[0_0_10px_rgba(244,63,94,0.5)]"><ThumbsUp size={10} className="fill-current" /></div>
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <button onClick={() => toggleLike(post)} className={`flex-1 flex items-center justify-center gap-2 py-2.5 sm:py-3 rounded-xl sm:rounded-[1rem] transition-all font-black text-xs ${post.isLikedByMe ? "text-rose-500 bg-rose-500/10 border border-rose-500/30 shadow-[0_0_15px_rgba(244,63,94,0.15)]" : "text-neutral-400 bg-white/[0.02] border border-transparent hover:bg-white/5 hover:text-white"}`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {enrollments.map((enr) => (
                        <div key={enr.id} className="bg-[#0a0a0f]/80 border border-white/10 p-4 sm:p-5 rounded-[1.5rem] flex items-center gap-4 hover:border-white/20 transition-all">
                          ...
                          <div className="flex-1">
                            <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest text-rose-500 bg-rose-500/10 px-2 py-1 rounded-md">{enr.courses?.category || 'Course'}</span>
                            ...
                            <div className="mt-2.5">
                              ...
                              <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden">
                                <div className="bg-rose-500 h-full rounded-full shadow-[0_0_8px_#f43f5e]" style={{ width: `${enr.progress_percentage}%` }}></div>
                              </div>
                            </div>
                          </div>
                        </div>
...
function InfoRow({ icon, label, value, isPrivate, isLink, className = "" }: any) {
  return (
    <div className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-white/5 border border-white/5 relative group hover:bg-white/10 hover:border-white/10 transition-colors ${className}`}>
      ...
      <div className="text-neutral-400 group-hover:text-rose-500 transition-colors">{icon}</div>
      ...
      elements.push(
        <div key={c.id} style={{ marginLeft: `${depth * 16}px` }} className="mb-4">
          <div className="flex gap-2.5 sm:gap-3 items-start">
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full sm:rounded-xl bg-neutral-800 shrink-0 border border-rose-500/20 flex items-center justify-center overflow-hidden">
              {c.profiles?.avatar_url ? <img src={c.profiles.avatar_url} className="w-full h-full object-cover" alt="" /> : <span className="text-rose-500 text-[10px] sm:text-xs font-black">{authorName.charAt(0)}</span>}
            </div>
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3 mt-1.5 ml-2">
                ...
                <button onClick={() => { setReplyingToId(c.id); setReplyingToName(authorName); }} className="text-[9px] sm:text-[10px] font-black text-rose-500 hover:text-rose-400 uppercase">Reply</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md animate-[fadeIn_0.2s_ease-out]">
      <div className="bg-[#0a0a0f] border border-white/10 rounded-t-[2rem] sm:rounded-[2rem] w-full max-w-2xl h-[85vh] sm:max-h-[90vh] flex flex-col shadow-[0_30px_60px_rgba(0,0,0,0.8)] overflow-hidden animate-[slideUp_0.3s_ease-out]">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 scrollbar-hide pb-32">
          {isLoading ? <div className="flex justify-center py-10"><Loader2 className="w-8 h-8 text-rose-500 animate-spin" /></div> : comments.length === 0 ? <div className="text-center py-16"><MessageSquare className="w-10 h-10 sm:w-12 sm:h-12 text-neutral-700 mx-auto mb-4" /><p className="text-neutral-500 font-bold text-xs sm:text-sm tracking-wide">No comments yet. Start the conversation!</p></div> : buildCommentTree(null, 0)}
        </div>
        <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5 sm:p-6 border-t border-white/5 bg-[#0a0a0f]/95 backdrop-blur-xl">
          {replyingToName && (
            <div className="flex items-center justify-between mb-2 sm:mb-3 px-3 py-1.5 sm:py-2 bg-rose-500/10 rounded-xl border border-rose-500/20">
              <span className="text-[10px] sm:text-xs font-black text-rose-500">Replying to {replyingToName}</span>
          ...
          <div className="flex items-end gap-2 sm:gap-3 pb-safe">
            <textarea value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder="Write your thought..." rows={1} className="flex-1 bg-neutral-900 border border-white/10 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3.5 sm:py-4 text-white text-xs sm:text-sm focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 resize-none min-h-[45px] sm:min-h-[55px] max-h-[100px] sm:max-h-[120px] transition-all" />
            <button onClick={handleSend} disabled={isSending || !newComment.trim()} className="w-[45px] h-[45px] sm:w-[55px] sm:h-[55px] shrink-0 bg-rose-500 text-white rounded-xl sm:rounded-2xl flex items-center justify-center hover:bg-rose-600 disabled:opacity-50 transition-all shadow-[0_0_15px_rgba(244,63,94,0.3)]">
