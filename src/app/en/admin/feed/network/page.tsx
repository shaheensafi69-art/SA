...
export default function AdminNetworkPage() {
  ...
  const fetchNetworkData = async () => {
    ...
    try {
      ...
      // 1. دریافت تمام کاربران به جز کاربر فعلی (ادمین)
      ...
      // 2. دریافت تمام روابط متصل به کاربر فعلی
  ...
  const getActionBtnProps = (status: string) => {
    switch (status) {
      ...
      case 'none':
      default:
        // رنگ دکمه اتصال به رنگ اصلی ادمین تغییر یافت
        return { text: "Connect", icon: <UserPlus size={16} />, style: "bg-rose-500 text-white border border-rose-500 hover:bg-rose-600 shadow-[0_0_15px_rgba(244,63,94,0.4)]" };
    }
  };
  ...
  return (
    <div className="max-w-[90rem] mx-auto px-4 sm:px-6 py-6 sm:py-10 font-sans relative min-h-screen">
      
      {/* هاله‌های نوری پس‌زمینه مخصوص ادمین */}
      <div className="absolute top-0 left-[20%] w-[40vw] h-[40vw] bg-rose-600/10 rounded-full blur-[140px] pointer-events-none z-0 animate-pulse" style={{ animationDuration: '7s' }}></div>
      ...
      <div className="relative z-10 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-[#0a0a0f]/80 border border-white/10 p-6 md:p-8 rounded-[2rem] backdrop-blur-xl shadow-2xl">
          <div className="flex items-center gap-5">
            <div className="w-14 h-14 bg-gradient-to-br from-rose-500 to-purple-600 rounded-[1.2rem] flex items-center justify-center text-white shadow-[0_10px_20px_rgba(244,63,94,0.3)] shrink-0">
              <Users size={28} />
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Academy Network</h1>
              <p className="text-xs sm:text-sm text-neutral-400 font-medium mt-1.5">Connect with peers, monitor students, and manage faculty.</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            <input
              ...
              className="w-full bg-[#0a0a0f]/60 border border-white/10 rounded-[1.5rem] px-5 py-4 pl-14 text-white placeholder-neutral-500 font-medium focus:outline-none focus:border-rose-500 transition-colors shadow-lg"
            />
        ...
        {filteredUsers.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-24">
            {filteredUsers.map((u) => {
              ...
              return (
                <div key={u.id} className="bg-[#0a0a0f]/80 border border-white/5 rounded-[2rem] p-6 flex flex-col items-center text-center backdrop-blur-md shadow-[0_15px_30px_rgba(0,0,0,0.4)] hover:border-rose-500/30 hover:-translate-y-1.5 transition-all duration-300 group">
                  
                  {/* Avatar */}
                  <Link href={`/en/admin/feed/profile/${u.id}`} className="relative mb-4 mt-2">
                    <div className="w-20 h-20 rounded-[1.5rem] bg-neutral-800 border-2 border-white/10 overflow-hidden flex items-center justify-center group-hover:border-rose-500/50 transition-colors relative z-10">
                      {u.avatar_url ? (
                        <img src={u.avatar_url} alt={u.first_name} className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-2xl font-black text-neutral-500 group-hover:text-rose-400 transition-colors">{u.first_name.charAt(0)}</span>
                      )}
                    </div>
                    {isFaculty && (
                      <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-purple-600 rounded-full flex items-center justify-center border-[3px] border-[#0a0a0f] z-20 shadow-lg">
                  ...
                  {/* Info */}
                  <Link href={`/en/admin/feed/profile/${u.id}`} className="block w-full">
                    <h3 className="text-lg font-black text-white truncate group-hover:text-rose-400 transition-colors">
                      {u.first_name} {u.last_name}
                    </h3>
                    <p className={`text-[10px] font-black uppercase tracking-widest mt-1 ${isFaculty ? 'text-purple-400' : 'text-neutral-500'}`}>
                      {u.role.replace('_', ' ')}
                    </p>
                  </Link>
