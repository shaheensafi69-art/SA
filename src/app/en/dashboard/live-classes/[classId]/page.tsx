...
export default function LiveMeetRoom() {
    ...
    const initMeeting = async () => {
      ...
      try {
        if (typeof window !== "undefined") {
          ...
          // مدیریت کاربران ورودی (همکلاسی‌ها / استاد)
  ...
  const toggleCamera = async () => {
    ...
    try {
      ...
      if (cameraOn) {
        ...
        if (screenSharing) {
          alert("Please stop screen sharing before turning on your camera.");
          return;
        }
        ...
        setTimeout(() => {
          if (localVideoContainerRef.current && localVideoTrackRef.current) {
            localVideoTrackRef.current.play(localVideoContainerRef.current, { fit: "cover" });
          }
        }, 100);
      }
    } catch (err) {
  ...
  // 🔥 هندلر پایدار اشتراک گذاری صفحه (Screen Share) 🔥
      ...
      if (screenSharing) {
        ...
        setTimeout(() => {
          if (localVideoContainerRef.current && localScreenTrackRef.current) {
            localScreenTrackRef.current.play(localVideoContainerRef.current, { fit: "contain" });
          }
        }, 100);
      }
    } catch (err) {
  ...
  // بررسی وضعیت پرزنتیشن برای چیدمان هوشمند Layout
  const isPresentationMode = screenSharing || remoteUsers.some(u => u._videoTrack?._trackMediaType === "screen");
  
  // محاسبه تعداد کل نفرات حاضر (خودمون + ریموت)
  const totalParticipants = 1 + remoteUsers.length;

  return (
    <div className="fixed inset-0 z-[100] lg:relative lg:inset-auto lg:z-10 lg:w-full lg:h-full bg-[#09090d] font-sans flex flex-col overflow-hidden select-none">
      
      {/* ================= HEADER ================= */}
      <header className="h-16 sm:h-20 px-4 sm:px-6 flex justify-between items-center bg-black/40 backdrop-blur-md border-b border-white/5 z-20 shrink-0 shadow-lg">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <p className="text-green-400 text-[9px] sm:text-[10px] uppercase tracking-widest font-bold">Encrypted Connection</p>
              </div>
            </div>
          </div>
        </div>

        <Link href={`/en/dashboard/groups/${classId}`} className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg transition-colors text-[10px] sm:text-xs uppercase tracking-widest shadow-lg">
      ...
      {/* ================= MAIN VIDEO AREA (Google Meet Style Layout) ================= */}
      <main className="flex-1 relative p-2 sm:p-4 flex flex-col overflow-hidden">
        {isLoading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <div className="w-12 h-12 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin mb-4"></div>
            <p className="text-indigo-400 text-xs font-black tracking-widest uppercase animate-pulse">Initializing Interface...</p>
          </div>
        ) : (
          <div className={`w-full h-full flex gap-3 transition-all duration-500 ${isPresentationMode ? "flex-col lg:flex-row" : "flex-row"}`}>
            
            {/* 🔥 کانتینر اصلی تصاویر (Grid تمام صفحه) 🔥 */}
            <div className={`transition-all duration-500 rounded-2xl overflow-hidden bg-[#16161e] border border-white/5 shadow-2xl flex-1
              ${!isPresentationMode && totalParticipants === 1 ? "flex items-center justify-center" : ""} 
              ${!isPresentationMode && totalParticipants > 1 ? `grid gap-2 grid-cols-1 sm:grid-cols-2 ${totalParticipants > 4 ? 'lg:grid-cols-3' : ''}` : ""}
            `}>
              
              {/* کادر تصویر شما (Local User) */}
              <div className={`relative w-full h-full min-h-[30vh] ${!isPresentationMode && totalParticipants === 1 ? "max-w-4xl max-h-[80vh] rounded-3xl overflow-hidden mx-auto" : "rounded-lg overflow-hidden bg-black"}`}>
                
                {/* نمای هنگام خاموش بودن کمره و اسکرین شیر */}
                {!(cameraOn || screenSharing) && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0d0d12]">
                    <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center text-white/20 mb-3 border border-white/5">
                    ...
                    <p className="text-neutral-500 text-xs font-bold uppercase tracking-widest">Camera Off</p>
                  </div>
                )}
                
                {/* Div پردازش تصویر آگورا (Cover برای کمره، Contain برای اسکرین‌شیر) */}
                <div 
                  ref={localVideoContainerRef} 
                  className={`absolute inset-0 w-full h-full [&>div]:w-full [&>div]:h-full ${screenSharing ? "[&>div>video]:object-contain" : "[&>div>video]:object-cover"}`}
                ></div>
              ...
              {/* رندر کردن افراد دیگر در حالت عادی (Grid) */}
              {!isPresentationMode && remoteUsers.map((user) => (
                <div key={user.uid} className="relative w-full h-full min-h-[30vh] rounded-lg overflow-hidden bg-black">
                  <div 
                  ...
                  ></div>
                  <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/10 flex items-center gap-2 z-10">
                    <span className="text-xs text-white font-bold tracking-wide">Participant</span>
                  </div>
                </div>
              ))}
            ...
            {/* 🔥 سایدبار کشویی افراد (فقط در حالت پرزنتیشن ظاهر می‌شود) 🔥 */}
            {isPresentationMode && remoteUsers.length > 0 && (
              <div className="w-full lg:w-64 xl:w-72 h-32 lg:h-full overflow-x-auto lg:overflow-y-auto flex lg:flex-col gap-2 custom-scrollbar pr-1">
                {remoteUsers.map((user) => (
                  <div key={user.uid} className="relative bg-[#16161e] shrink-0 w-48 lg:w-full h-full lg:h-40 rounded-xl border border-white/5 overflow-hidden shadow-lg">
                    <div 
                      className="absolute inset-0 w-full h-full [&>div]:w-full [&>div]:h-full [&>div>video]:object-cover"
                      ref={(el) => {
                        if (el && user.videoTrack && !el.hasChildNodes()) {
                    ...
                    <div className="absolute bottom-2 left-2 bg-black/60 backdrop-blur-md px-2 py-1 rounded-md border border-white/10 z-10">
                      <span className="text-[10px] text-white font-bold">Participant</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* ================= FLOATING CONTROLS (منظم و واکنش‌گرا) ================= */}
      <footer className="h-24 pb-4 flex justify-center items-center shrink-0 z-20 pointer-events-none relative">
        <div className="bg-[#111116]/95 backdrop-blur-2xl border border-white/10 px-4 sm:px-6 py-2.5 sm:py-3 rounded-full flex items-center gap-3 sm:gap-5 shadow-[0_20px_50px_rgba(0,0,0,0.8)] pointer-events-auto transition-transform hover:scale-[1.02]">
          ...
          {/* اسکرین شیر */}
          <button onClick={toggleScreenShare} className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center transition-all ${screenSharing ? "bg-indigo-500 text-white shadow-[0_0_20px_rgba(99,102,241,0.6)]" : "bg-white/5 hover:bg-white/10 text-neutral-400 hover:text-white"}`}>
          ...
          {/* خروج */}
          <button onClick={handleLeave} className="px-6 sm:px-8 h-12 sm:h-14 rounded-full bg-red-600 hover:bg-red-500 text-white flex items-center justify-center font-bold text-xs tracking-widest uppercase transition-all shadow-[0_0_15px_rgba(220,38,38,0.5)]">
            Leave
          </button>
