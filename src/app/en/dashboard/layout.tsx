...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isReady) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#020202] text-white">
         ...
         <span className="text-yellow-500 font-bold uppercase tracking-widest text-sm animate-pulse">Loading Academy...</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-[#020202] text-white font-sans overflow-hidden relative">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[40vw] h-[40vw] bg-yellow-600/10 rounded-full blur-[120px] pointer-events-none z-0"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[30vw] h-[30vw] bg-amber-600/5 rounded-full blur-[100px] pointer-events-none z-0"></div>
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-4 px-4 py-3.5 rounded-2xl font-bold text-sm transition-all duration-300 relative overflow-hidden group ${
                  isActive ? "text-black shadow-[0_10px_20px_rgba(234,179,8,0.15)] scale-[1.02]" : "text-neutral-400 hover:bg-white/5 hover:text-white"
                }`}
              >
        ...
        <div className="p-5 border-t border-white/5 bg-black/40 shrink-0">
          <div className="bg-white/5 border border-white/10 p-3 rounded-2xl flex items-center justify-between hover:bg-white/10 transition-colors">
            <div className="flex items-center gap-3 overflow-hidden">
              <div className="w-10 h-10 rounded-xl border border-yellow-500/30 overflow-hidden shrink-0 bg-neutral-900 flex items-center justify-center">
          ...
          {[
            ...
            return (
              <Link key={item.path} href={item.path} className="relative flex flex-col items-center justify-center w-16 h-full group">
                {isActive && <div className="absolute inset-0 bg-gradient-to-t from-yellow-500/15 to-transparent rounded-full opacity-100 transition-opacity duration-300"></div>}
                <span className={`text-xl transition-all duration-500 ease-out z-10 ${isActive ? "-translate-y-2 scale-110 drop-shadow-[0_0_8px_rgba(234,179,8,0.5)]" : "text-neutral-500 group-hover:-translate-y-1 group-hover:text-neutral-300"}`}>{item.icon}</span>
                <span className={`absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all duration-500 z-10 ${isActive ? "text-yellow-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0"}`}>{item.name}</span>
                {isActive && <div className="absolute bottom-1 w-1 h-1 rounded-full bg-yellow-400 shadow-[0_0_5px_#facc15]"></div>}
              </Link>
            );
          })}
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-[#020202]/95 backdrop-blur-3xl z-[100] flex flex-col animate-[fadeIn_0.2s_ease-out] lg:hidden">
          <div className="h-16 px-5 border-b border-white/5 flex justify-between items-center bg-black/40 shrink-0">
            <span className="font-black text-white uppercase tracking-widest text-xs">All Tools & Features</span>
