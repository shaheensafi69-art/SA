...
import {
  ...
  Rss, Users, SquarePen, User, Megaphone, BookOpen, Video,
  FileText, Target, TrendingUp, Trophy, Settings, Grid, Headset
} from "lucide-react";
...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  const isSupportChatRoute = pathname.includes('/en/teacher/support/chat');
  const isFullScreenRoute = isGroupChatRoute || isLiveMeetRoute || isSupportChatRoute;

  // بررسی اینکه آیا استاد در بخش فید/سوشال قرار دارد
  const isSocialRoute = pathname.includes('/teacher/feed');
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      if (user && !userError) {
        ...
        if (profile) {
          // محافظت از مسیر: فقط اساتید و ادمین‌ها حق ورود دارند
  ...
  const myProfilePath = userProfile?.id ? `/en/teacher/feed/profile/${userProfile.id}` : "/en/teacher/feed/profile";

  // لیست منوها با اضافه شدن دقیق فید و پشتیبانی با مسیرهای استادی
  const menuItems = [
    { name: "Overview", path: "/en/teacher", icon: <LayoutDashboard size={22} />, disabled: false },
    { name: "Academy Feed", path: "/en/teacher/feed", icon: <Rss size={22} />, disabled: false },
    { name: "Manage Network", path: "/en/teacher/feed/network", icon: <Users size={22} />, disabled: false },
    { name: "Create Post", path: "/en/teacher/feed/create", icon: <SquarePen size={22} />, disabled: false },
    { name: "My Profile", path: myProfilePath, icon: <User size={22} />, disabled: false },
    ...
    { name: "My Students", path: "/en/teacher/students", icon: <Users size={22} />, disabled: false },
    ...
    { name: "Support Tickets", path: "/en/teacher/support", icon: <Headset size={22} />, disabled: false },
  ...
  return (
    <div className="flex h-screen bg-[#030305] text-white font-sans overflow-hidden relative selection:bg-fuchsia-500 selection:text-white">
      ...
      {/* هاله‌های نوری پویا با طیف فوشیا/بنفش اساتید */}
        ...
        <nav className="flex-1 overflow-y-auto space-y-2 custom-scrollbar pr-1">
          {menuItems.map((item) => {
            ...
            const isActive = item.name === "My Profile"
              ? pathname.includes("/teacher/feed/profile")
              : item.path === "/en/teacher"
                ? pathname === "/en/teacher"
        ...
        <div className="mt-4 pt-4 border-t border-white/[0.06] space-y-3 shrink-0">
          <div className="bg-[#09090e] border border-white/[0.05] p-3 rounded-2xl flex items-center justify-between shadow-[inset_0_2px_5px_rgba(0,0,0,0.8)] relative overflow-hidden group">
            ...
            <div className="flex items-center gap-3 overflow-hidden relative z-10">
              <div className="w-10 h-10 rounded-xl border border-fuchsia-500/40 overflow-hidden shrink-0 bg-neutral-900 flex items-center justify-center shadow-inner group-hover:border-fuchsia-500 transition-colors">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[9px] text-fuchsia-400 font-black tracking-widest uppercase mt-0.5">
                  Instructor
                </p>
              </div>
            </div>
      ...
      {!isFullScreenRoute && (
        <div className="lg:hidden fixed top-0 left-0 right-0 h-16 flex items-center justify-between px-5 bg-[#030305]/90 backdrop-blur-2xl border-b border-white/[0.06] z-40 shadow-md">
          <div className="flex items-center gap-3 relative">
            ...
            <img src="/logo-without-b.png" alt="Safi Academy" className="relative z-10 w-8 h-8 object-contain filter drop-shadow-[0_0_8px_rgba(217,70,239,0.6)]" />
          ...
          <Link href="/en/teacher/announcements" className="w-10 h-10 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-neutral-400 hover:text-fuchsia-400 transition-colors relative shadow-inner">
            <Bell size={18} />
            <span className="absolute top-2 right-2 w-1.5 h-1.5 bg-fuchsia-500 rounded-full animate-pulse shadow-[0_0_6px_#d946ef]"></span>
          </Link>
        </div>
      )}
      ...
      {!isFullScreenRoute && (
        <div className="lg:hidden fixed bottom-6 left-4 right-4 h-[75px] bg-[#09090e]/95 backdrop-blur-3xl border border-white/10 z-50 px-2 rounded-[2.5rem] flex justify-between items-center shadow-[0_20px_50px_rgba(0,0,0,0.9),0_0_20px_rgba(217,70,239,0.05)]">
          {isSocialRoute ? (
            <>
              {/* FEED NAV */}
              <Link href="/en/teacher/feed" className="relative flex flex-col items-center justify-center w-[20%] h-full group">
                {pathname === "/en/teacher/feed" && <div className="absolute inset-0 bg-gradient-to-t from-fuchsia-500/20 to-transparent rounded-[2rem] opacity-100"></div>}
                <Rss size={22} className={`z-10 transition-all ${pathname === "/en/teacher/feed" ? "-translate-y-2 text-fuchsia-400" : "text-neutral-500 group-hover:text-neutral-300"}`} />
                <span className={`absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all z-10 ${pathname === "/en/teacher/feed" ? "text-fuchsia-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2"}`}>Feed</span>
              </Link>
              {/* NETWORK NAV */}
              <Link href="/en/teacher/feed/network" className="relative flex flex-col items-center justify-center w-[20%] h-full group">
                {pathname.includes("/teacher/feed/network") && <div className="absolute inset-0 bg-gradient-to-t from-indigo-500/20 to-transparent rounded-[2rem] opacity-100"></div>}
                <Users size={22} className={`z-10 transition-all ${pathname.includes("/teacher/feed/network") ? "-translate-y-2 text-indigo-400" : "text-neutral-500 group-hover:text-neutral-300"}`} />
                <span className={`absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all z-10 ${pathname.includes("/teacher/feed/network") ? "text-indigo-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2"}`}>Network</span>
              </Link>
              {/* CREATE POST (FAB CENTER) */}
              <Link href="/en/teacher/feed/create" className="relative flex flex-col items-center justify-start w-[20%] h-full group -mt-8">
                <div className="absolute top-1 w-16 h-16 bg-fuchsia-500/40 blur-[12px] rounded-full group-hover:bg-fuchsia-500/60 transition-all duration-300"></div>
                <div className="relative z-10 w-[56px] h-[56px] bg-gradient-to-br from-fuchsia-500 to-purple-600 rounded-full flex items-center justify-center border-[5px] border-[#09090e] shadow-xl group-hover:scale-105 transition-transform duration-300">
                  <SquarePen size={22} className="text-white ml-0.5" strokeWidth={2.5} />
                </div>
                <span className="text-[9px] font-black tracking-widest uppercase text-fuchsia-500 mt-1.5 drop-shadow-[0_0_8px_#d946ef]">Post</span>
              </Link>
              {/* PROFILE NAV */}
              <Link href={myProfilePath} className="relative flex flex-col items-center justify-center w-[20%] h-full group">
                {pathname.includes("/teacher/feed/profile") && <div className="absolute inset-0 bg-gradient-to-t from-blue-500/20 to-transparent rounded-[2rem] opacity-100"></div>}
                <User size={22} className={`z-10 transition-all ${pathname.includes("/teacher/feed/profile") ? "-translate-y-2 text-blue-400" : "text-neutral-500 group-hover:text-neutral-300"}`} />
                <span className={`absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all z-10 ${pathname.includes("/teacher/feed/profile") ? "text-blue-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2"}`}>Profile</span>
              </Link>
              {/* MENU NAV */}
            ...
            <>
              ...
              <Link href="/en/teacher" className="relative flex flex-col items-center justify-center w-[25%] h-full group">
                {pathname === "/en/teacher" && <div className="absolute inset-0 bg-gradient-to-t from-fuchsia-500/20 to-transparent rounded-[2rem] opacity-100"></div>}
                <LayoutDashboard size={22} className={`z-10 transition-all ${pathname === "/en/teacher" ? "-translate-y-2 text-fuchsia-400" : "text-neutral-500 group-hover:text-neutral-300"}`} />
                <span className={`absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all z-10 ${pathname === "/en/teacher" ? "text-fuchsia-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2"}`}>Overview</span>
              </Link>
              {/* COURSES NAV */}
              <Link href="/en/teacher/courses" className="relative flex flex-col items-center justify-center w-[25%] h-full group">
                <BookOpen size={22} className={`z-10 transition-all ${pathname.includes("/teacher/courses") ? "-translate-y-2 text-emerald-400" : "text-neutral-500 group-hover:text-neutral-300"}`} />
                <span className={`absolute bottom-2 text-[9px] font-black tracking-widest uppercase transition-all z-10 ${pathname.includes("/teacher/courses") ? "text-emerald-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2"}`}>Courses</span>
              </Link>
              {/* FEED ENTRY NAV */}
              <Link href="/en/teacher/feed" className="relative flex flex-col items-center justify-center w-[25%] h-full group">
        ...
        <div className="fixed inset-0 bg-[#030305]/95 backdrop-blur-3xl z-[100] flex flex-col animate-[fadeIn_0.2s_ease-out] lg:hidden">
          <div className="h-16 px-5 border-b border-white/[0.06] flex justify-between items-center bg-black/50 shrink-0">
            <span className="font-black text-white uppercase tracking-widest text-xs flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-fuchsia-500 animate-ping"></span> Instructor Tools
            </span>
              ...
              {menuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.name === "Live Classes" && <div className="absolute top-3 right-3 w-2 h-2 bg-red-500 rounded-full animate-pulse shadow-[0_0_8px_#ef4444]"></div>}
                  </Link>
                );
              })}
            </div>
