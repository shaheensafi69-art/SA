...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    ...
    { name: "Announcements", path: "/en/dashboard/announcements", icon: "📢" },
    ...
    { name: "AI Assistant", path: "#", icon: "🤖", disabled: true },
  ...
  if (!isReady) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#020202] text-white">
         <div className="w-16 h-16 border-4 border-yellow-500 border-t-transparent rounded-full animate-spin mb-4 shadow-[0_0_25px_rgba(234,179,8,0.4)]"></div>
         <span className="text-yellow-500 font-bold uppercase tracking-widest text-xs animate-pulse">Initializing Student Portal...</span>
      </div>
    );
  }

  return (
    <div className="flex h-screen bg-[#030305] text-white font-sans overflow-hidden relative selection:bg-yellow-500 selection:text-black">
      
      {/* 🌟 هاله‌های نوری پویا و زنده‌ی پس‌زمینه با رنگ طلایی اکادمی */}
      <div className="absolute top-[-15%] left-[-15%] w-[50vw] h-[50vw] bg-yellow-600/15 rounded-full blur-[140px] pointer-events-none z-0 animate-pulse" style={{ animationDuration: '8s' }}></div>
      <div className="absolute bottom-[-15%] right-[-15%] w-[40vw] h-[40vw] bg-amber-600/10 rounded-full blur-[130px] pointer-events-none z-0 animate-pulse" style={{ animationDuration: '6s' }}></div>
      ...
      <aside className="hidden lg:flex w-[295px] bg-[#060609]/90 backdrop-blur-2xl border-r border-white/[0.06] flex-col relative z-20 shrink-0 p-4 shadow-[15px_0_40px_rgba(0,0,0,0.9)]">
        ...
        {/* کپسول بالای سایدبار با لوگو و افکت درخشش طلایی */}
        <div className="mb-5 p-4 rounded-2xl bg-gradient-to-br from-[#0c0c14] to-[#07070a] shadow-[inset_0_2px_6px_rgba(0,0,0,0.9),0_6px_20px_rgba(0,0,0,0.5)] border border-yellow-500/10 shrink-0 relative group">
          <div className="absolute -inset-0.5 bg-gradient-to-r from-yellow-500 to-amber-600 rounded-2xl opacity-20 blur group-hover:opacity-45 transition duration-500"></div>
          <Link href="/en/dashboard" className="flex items-center gap-3.5 relative z-10">
             <div className="relative flex items-center justify-center transform group-hover:rotate-6 group-hover:scale-110 transition-transform duration-300">
               <div className="absolute inset-0 bg-yellow-500/30 blur-[10px] rounded-full"></div>
               <img src="/logo-without-b.png" alt="Safi Academy" className="relative z-10 w-10 h-10 object-contain drop-shadow-[0_0_10px_rgba(234,179,8,0.7)]" />
             </div>
             <div>
               <h2 className="text-xs font-black text-white tracking-widest uppercase flex items-center gap-1.5">
                 Safi Academy <Sparkles size={10} className="text-yellow-400 animate-spin" style={{ animationDuration: '4s' }} />
               </h2>
               <p className="text-[9px] text-yellow-500 font-bold uppercase tracking-[0.2em] mt-0.5">Student Portal</p>
             </div>
          </Link>
        </div>
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-4 px-4 py-3 rounded-2xl font-bold text-xs transition-all duration-300 relative group overflow-hidden ${
                  isActive 
                    ? "bg-gradient-to-r from-[#1c1a12] to-[#0a0a0e] text-white shadow-[inset_0_2px_5px_rgba(0,0,0,0.9),0_0_20px_rgba(234,179,8,0.2)] border border-yellow-500/40 translate-x-1" 
              ...
              >
                {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-yellow-400 rounded-r-full shadow-[0_0_10px_#facc15]"></div>}
                <span className={`text-lg transition-all duration-300 ${isActive ? "scale-125 text-yellow-400 rotate-3" : "group-hover:scale-110 group-hover:text-white"}`}>{item.icon}</span>
                <span className="tracking-wide">{item.name}</span>
        ...
        <div className="mt-4 pt-4 border-t border-white/[0.06] space-y-3 shrink-0">
          <div className="bg-[#09090e] border border-white/[0.05] p-3 rounded-2xl flex items-center justify-between shadow-[inset_0_2px_5px_rgba(0,0,0,0.8)] relative overflow-hidden group">
            ...
            <div className="flex items-center gap-3 overflow-hidden relative z-10">
              <div className="w-10 h-10 rounded-xl border border-yellow-500/40 overflow-hidden shrink-0 bg-neutral-900 flex items-center justify-center shadow-inner group-hover:border-yellow-400 transition-colors">
                {userProfile?.avatar_url ? (
                  ...
                  <span className="text-yellow-500 font-bold">{userProfile?.first_name?.charAt(0) || "S"}</span>
                )}
              </div>
      ...
      <div className="hidden lg:flex absolute top-6 right-10 z-50">
         <Link href="/en/dashboard/announcements" className="w-12 h-12 bg-[#0a0a0f]/80 backdrop-blur-xl border border-white/10 rounded-2xl flex items-center justify-center text-neutral-400 hover:text-yellow-400 hover:border-yellow-500/40 transition-all relative group shadow-[0_10px_25px_rgba(0,0,0,0.5)]">
           ...
           <span className="absolute -inset-0.5 bg-yellow-500/20 rounded-2xl blur opacity-0 group-hover:opacity-100 transition-opacity"></span>
         </Link>
      </div>
      ...
      {!isFullScreenRoute && (
        <div className="lg:hidden fixed top-0 left-0 right-0 h-16 flex items-center justify-between px-5 bg-[#030305]/90 backdrop-blur-2xl border-b border-white/[0.06] z-40">
          <div className="flex items-center gap-3 relative">
            ...
            <img src="/logo-without-b.png" alt="Safi Academy" className="relative z-10 w-9 h-9 object-contain filter drop-shadow-[0_0_8px_rgba(234,179,8,0.6)]" />
            <span className="relative z-10 font-black text-xs tracking-widest text-white uppercase">Safi Academy</span>
          </div>
          
          <Link href="/en/dashboard/announcements" className="w-10 h-10 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-neutral-400 hover:text-yellow-400 transition-colors relative shadow-inner">
      ...
      {!isFullScreenRoute && (
        <div className="lg:hidden fixed bottom-6 left-5 right-5 h-[70px] bg-[#09090e]/90 backdrop-blur-3xl border border-white/10 z-50 px-3 rounded-full flex justify-around items-center shadow-[0_20px_50px_rgba(0,0,0,0.9),0_0_25px_rgba(234,179,8,0.1)]">
          {[
            { name: "Overview", path: "/en/dashboard", icon: "📊" },
            { name: "Courses", path: "/en/dashboard/courses", icon: "📚" },
            { name: "Live", path: "/en/dashboard/live-classes", icon: "🔴" },
          ].map((item) => {
            const isActive = item.path === "/en/dashboard" ? pathname === "/en/dashboard" : pathname.startsWith(item.path);
            return (
              <Link key={item.path} href={item.path} className="relative flex flex-col items-center justify-center w-16 h-full group">
                {isActive && <div className="absolute inset-0 bg-gradient-to-t from-yellow-500/20 to-transparent rounded-full opacity-100 transition-all duration-300"></div>}
                <span className={`text-xl transition-all duration-300 ease-out z-10 ${isActive ? "-translate-y-2.5 scale-125 drop-shadow-[0_0_10px_rgba(234,179,8,0.6)]" : "text-neutral-500 group-hover:-translate-y-1 group-hover:text-neutral-300"}`}>{item.icon}</span>
                <span className={`absolute bottom-1.5 text-[9px] font-black tracking-widest uppercase transition-all duration-300 z-10 ${isActive ? "text-yellow-400 opacity-100 translate-y-0" : "text-neutral-500 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0"}`}>{item.name}</span>
                {isActive && <div className="absolute bottom-1 w-1 h-1 rounded-full bg-yellow-400 shadow-[0_0_6px_#facc15]"></div>}
              </Link>
            );
          })}
        ...
        <div className="fixed inset-0 bg-[#030305]/95 backdrop-blur-3xl z-[100] flex flex-col animate-[fadeIn_0.2s_ease-out] lg:hidden">
          <div className="h-16 px-5 border-b border-white/[0.06] flex justify-between items-center bg-black/50 shrink-0">
            <span className="font-black text-white uppercase tracking-widest text-xs flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-yellow-400 animate-ping"></span> All Tools & Features
            </span>
                ...
                return (
                  <Link
                    ...
                    className={`relative flex flex-col items-center justify-center gap-3 p-5 rounded-[1.8rem] font-bold transition-all border bg-gradient-to-br hover:scale-105 active:scale-95 shadow-lg ${colorClasses} ${
                      isActive ? "ring-2 ring-yellow-400/50 shadow-[0_0_20px_rgba(234,179,8,0.3)] opacity-150 scale-102" : "opacity-90 hover:opacity-100"
                    }`}
                  >
                    <span className="text-3xl drop-shadow-md">{item.icon}</span>
                    <span className="text-[10px] tracking-widest uppercase text-center line-clamp-1">{item.name}</span>
            ...
            <div className="mt-8 pt-6 border-t border-white/10 space-y-4">
               {userProfile && (
                <div className="flex items-center gap-4 bg-white/[0.03] border border-white/[0.06] p-4 rounded-3xl shadow-inner">
                  <img src={userProfile.avatar_url || "https://i.pravatar.cc/150"} alt="User" className="w-12 h-12 rounded-xl border border-yellow-500/50 object-cover shadow-md" />
