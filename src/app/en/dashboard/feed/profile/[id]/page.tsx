...
import { 
  ...
  Award, Flame, Wallet, Calendar, Share2, Link as LinkIcon, Activity
} from "lucide-react";
  ...
  if (!profileData) {
    return (
      <div className="w-full h-[80vh] flex flex-col items-center justify-center">
        ...
        <Link href="/en/dashboard/feed" className="mt-4 text-[#C2185B] hover:underline font-bold">Return to Feed</Link>
      </div>
    );
  }
              ...
              {activeTab === 'posts' && (
                <div className="space-y-4 sm:space-y-6">
                  {posts.length === 0 ? (
                    ...
                    posts.map((post) => (
                      <div key={post.id} className="bg-[#0a0a0f]/80 sm:border border-white/10 rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-8 backdrop-blur-md space-y-4 sm:space-y-5 shadow-lg sm:shadow-[0_15px_35px_rgba(0,0,0,0.5)]">
                        {/* Header */}
