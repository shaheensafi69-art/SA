...
export default function CreatePostPage() {
  ...
  const [selectedMood, setSelectedMood] = useState("Analysis");
  ...
  // دسته‌بندی‌های کاملاً حرفه‌ای و مناسب برای یک آکادمی
  const moods = [
    "Analysis",
    "Trade Setup",
    "Education",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ۲. ساخت عنوان نهایی به همراه تگ
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-8 py-8 sm:py-12 font-sans relative min-h-screen">
      ...
      <form onSubmit={handleSubmit} className="bg-[#0a0a0f]/80 border border-white/10 rounded-[2.5rem] p-6 sm:p-10 backdrop-blur-2xl shadow-[0_20px_40px_rgba(0,0,0,0.5)] relative overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#C2185B]/5 rounded-full blur-[80px] pointer-events-none"></div>
        ...
        <div className="space-y-8 relative z-10">
            ...
            <div className="flex flex-wrap gap-3">
              {moods.map((m) => (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 border ${selectedMood === m
                    ? "bg-[#C2185B]/10 text-[#C2185B] border-[#C2185B]/40 shadow-[0_0_15px_rgba(194,24,91,0.2)]"
                    : "bg-[#030305]/50 text-neutral-500 border-white/5 hover:border-white/20 hover:text-neutral-300"
                    }`}
                >
                  {selectedMood === m && <CheckCircle2 size={14} className="text-[#C2185B]" />}
                  {m}
                </button>
              ))}
            </div>
          ...
          <div className="space-y-3">
            ...
            <textarea
              ...
              rows={8}
              className="w-full bg-[#030305]/60 border border-white/10 rounded-[1.5rem] px-6 py-5 text-white placeholder-neutral-600 focus:outline-none focus:border-[#C2185B] focus:ring-1 focus:ring-[#C2185B] transition-all shadow-inner resize-none text-sm leading-relaxed"
              required
            />
          </div>
          ...
          <div className="space-y-3">
            ...
            {!imagePreview ? (
              <div
                ...
                className="w-full border-2 border-dashed border-white/10 hover:border-[#C2185B]/50 rounded-[1.5rem] p-10 flex flex-col items-center justify-center gap-4 bg-[#030305]/40 hover:bg-[#C2185B]/5 transition-all cursor-pointer group"
              >
                <div className="w-16 h-16 rounded-full bg-white/5 group-hover:bg-[#C2185B]/10 flex items-center justify-center transition-colors">
          ...
          <div className="flex items-center justify-end pt-2">
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-3 px-10 py-4 bg-gradient-to-r from-[#C2185B] to-pink-700 text-white font-black text-xs uppercase tracking-widest rounded-xl hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:hover:scale-100 shadow-[0_10px_25px_rgba(194,24,91,0.4)]"
            >
              {isSubmitting ? (
                <>
