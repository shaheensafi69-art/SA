        ...
        <header className="flex flex-col md:flex-row justify-between md:items-center gap-6 bg-[#0a0a0f]/80 p-6 sm:p-8 rounded-[2rem] border border-white/5 backdrop-blur-2xl shadow-xl">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white mb-1">
              {t.teacherPages.initializeCohort} <span className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-purple-500">{t.teacherPages.cohortWord}</span>
            </h1>
            <p className="text-xs sm:text-sm text-neutral-400 font-medium">{t.teacherPages.configureLiveClassroomDesc}</p>
          </div>
        </header>
        ...
        <form onSubmit={handleSubmit} className="bg-neutral-900/40 border border-white/5 rounded-[2.5rem] p-6 sm:p-10 backdrop-blur-3xl shadow-2xl space-y-8">
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-neutral-400 uppercase tracking-widest ml-1">{t.teacherPages.parentCourse} *</label>
                {courses.length === 0 ? (
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2 pt-2">
              ...
              <input type="text" placeholder={t.teacherPages.scheduleSummaryPlaceholder} value={form.schedule_info} onChange={(e) => setForm({...form, schedule_info: e.target.value})} className="w-full bg-black/60 border border-white/10 rounded-2xl px-4 py-4 text-white text-sm focus:outline-none focus:border-fuchsia-500/50" />
            </div>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input type="url" placeholder={t.teacherPages.meetingLinkPlaceholder} value={form.meeting_link} onChange={(e) => setForm({...form, meeting_link: e.target.value})} className="w-full bg-black/60 border border-white/10 rounded-2xl pl-11 pr-4 py-4 text-white text-sm focus:outline-none focus:border-fuchsia-500/50" />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input type="url" placeholder={t.teacherPages.supportGroupPlaceholder} value={form.signal_group_link} onChange={(e) => setForm({...form, signal_group_link: e.target.value})} className="w-full bg-black/60 border border-white/10 rounded-2xl pl-11 pr-4 py-4 text-white text-sm focus:outline-none focus:border-fuchsia-500/50" />
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-5 mt-4 bg-white/[0.02] border border-white/5 rounded-2xl">
              <div>
                <p className="font-black text-white text-sm">{t.teacherPages.activateCohort}</p>
                <p className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest mt-0.5">{t.teacherPages.allowStudentsSeeLinks}</p>
              </div>
              <button 
                type="button" 
