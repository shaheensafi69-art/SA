...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  const isSupportChatRoute = pathname.includes('/support/chat');
    ...
    <div className="flex h-screen bg-[#030305] text-white font-sans overflow-hidden relative selection:bg-fuchsia-500 selection:text-white">
      ...
      <aside className="hidden lg:flex w-[295px] bg-[#060609]/90 backdrop-blur-2xl border-r border-white/[0.06] flex-col relative z-20 shrink-0 p-4 shadow-[15px_0_40px_rgba(0,0,0,0.9)]">
        ...
        <div className="mb-5 p-4 rounded-2xl bg-gradient-to-br from-[#0c0c14] to-[#07070a] shadow-[inset_0_2px_6px_rgba(0,0,0,0.9),0_6px_20px_rgba(0,0,0,0.5)] border border-fuchsia-500/10 shrink-0 relative group">
          ...
          <Link href={`/${currentLocale}/teacher`} className="flex items-center gap-3.5 relative z-10">
        ...
        <nav className="flex-1 overflow-y-auto space-y-2 custom-scrollbar pr-1">
          {menuItems.map((item) => {
            ...
            const isActive = item.path === `/${currentLocale}/teacher`
              ? pathname === `/${currentLocale}/teacher`
          ...
          <div className="bg-[#09090e] border border-white/[0.05] p-3 rounded-2xl flex items-center justify-between shadow-[inset_0_2px_5px_rgba(0,0,0,0.8)] relative overflow-hidden group">
            ...
            <div className="flex items-center gap-3 overflow-hidden relative z-10">
              <div className="w-10 h-10 rounded-xl border border-fuchsia-500/40 overflow-hidden shrink-0 bg-neutral-900 flex items-center justify-center shadow-inner group-hover:border-fuchsia-500 transition-colors">
                {userProfile?.avatar_url ? <img src={userProfile.avatar_url} alt="Teacher" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" /> : <span className="text-fuchsia-500 font-bold">{userProfile?.first_name?.charAt(0) || "T"}</span>}
              </div>
              <div className="min-w-0">
                <p className="text-xs font-bold text-white truncate flex items-center gap-1.5">{userProfile?.first_name} {userProfile?.last_name} <ShieldCheck size={12} className="text-fuchsia-400 shrink-0" /></p>
                <p className="text-[9px] text-fuchsia-400 font-black tracking-widest uppercase mt-0.5">{userProfile?.role?.replace('_', ' ') || 'Instructor'}</p>
              </div>
            </div>
          </div>
      ...
      {!isFullScreenRoute && (
        <div className="lg:hidden fixed top-0 left-0 right-0 h-16 flex items-center justify-between px-5 bg-[#030305]/90 backdrop-blur-2xl border-b border-white/[0.06] z-40 shadow-md">
          ...
          <Link href={`/${currentLocale}/teacher/announcements`} className="w-10 h-10 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-neutral-400 hover:text-fuchsia-400">
            <Bell size={18} />
          </Link>
        </div>
      )}
