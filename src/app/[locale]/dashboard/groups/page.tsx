...
type GroupCard = {
  id: string;
  class_name: string;
  signal_group_link: string | null; // اضافه شدن لینک امن سیگنال
...
export default function MyGroupsPage() {
  ...
  useEffect(() => {
    ...
    // اتصال ریل‌تایم برای گوش دادن به تابلو اعلانات کلاس‌ها
  ...
  const fetchMyGroups = async () => {
    ...
    try {
      ...
      // دریافت اطلاعات همراه با فیلد پیوند سیگنال
      let query = supabase.from("class_groups").select(`
        id, class_name, signal_group_link,
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              {filteredGroups.map((group) => {
                ...
                return (
                  group.signal_group_link ? (
                    /* حالت فعال: هدایت مستقیم به لینک سیگنال */
                    <a 
                      href={group.signal_group_link} 
                      target="_blank" 
                    ...
                    >
                      ...
                      <div className="relative bg-neutral-900/40 border border-white/5 p-5 sm:p-7 rounded-[2.5rem] backdrop-blur-3xl transition-all duration-300 group-hover:border-white/20 group-hover:-translate-y-1 flex items-center justify-between gap-6 shadow-2xl">
                        <div className="flex items-center gap-6 flex-1 min-w-0">
                          {/* Avatar Display */}
                          <div className="relative shrink-0">
                            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-[1.8rem] bg-gradient-to-br from-indigo-950 to-black border border-indigo-500/20 flex items-center justify-center text-3xl font-black shadow-inner group-hover:scale-105 transition-transform duration-500 text-indigo-400">
                              {group.class_name.charAt(0)}
                            </div>
                            <span className="absolute -bottom-1 -right-1 flex h-5 w-5 bg-blue-500 rounded-full items-center justify-center border-2 border-[#020202] text-[10px]">💬</span>
                          </div>
                            ...
                            <div className="flex items-center gap-3 overflow-hidden">
                              <div className="flex items-center gap-2 px-3 py-1 bg-white/5 border border-white/5 rounded-full shrink-0">
                                ...
                                <span className="text-[10px] font-black text-neutral-400 uppercase tracking-widest">
                                  {group.teacher?.first_name || "Faculty"}
                                </span>
                              </div>
                              
                              <p className="text-sm text-neutral-500 truncate italic">
                                {group.latest_message ? group.latest_message.message_text : "Enter protected Signal group channel..."}
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        {/* Right Trigger Arrow */}
                        <div className="hidden lg:flex w-14 h-14 rounded-2xl bg-white/5 border border-white/5 items-center justify-center text-neutral-400 group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-500 transition-all group-hover:rotate-[360deg] duration-700 shrink-0">
                          <ExternalLink size={18} />
                        </div>
                      </div>
                    </a>
                  ) : (
                    /* حالت غیرفعال: گروه هنوز ست نشده است */
                    <div key={group.id} className="relative bg-neutral-900/20 border border-white/5 opacity-40 rounded-[2.5rem] p-5 sm:p-7 flex items-center justify-between gap-6 cursor-not-allowed">
                      <div className="flex items-center gap-6">
                        <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-[1.8rem] bg-neutral-900 border border-white/5 flex items-center justify-center text-3xl font-black text-neutral-600">
                          {group.class_name.charAt(0)}
                        </div>
                        <div>
