...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    { id: "overview", name: t.dashboard.overview, path: `/${currentLocale}/dashboard`, icon: <LayoutDashboard size={22} /> },
    { id: "feed", name: t.dashboard.feed, path: `/${currentLocale}/feed`, icon: <Rss size={22} /> },
    { id: "profile", name: t.dashboard.myProfile, path: myProfilePath, icon: <User size={22} /> },
    { id: "announcements", name: t.dashboard.announcements, path: `/${currentLocale}/dashboard/announcements`, icon: <Megaphone size={22} /> },
    { id: "liveCampus", name: t.dashboard.liveCampus, path: `/${currentLocale}/dashboard/live-classes`, icon: <Video size={22} /> },
    { id: "assignments", name: t.dashboard.assignments, path: `/${currentLocale}/dashboard/assignments`, icon: <FileText size={22} /> },
    { id: "quizzes", name: t.dashboard.quizzes, path: `/${currentLocale}/dashboard/quizzes`, icon: <Target size={22} /> },
    { id: "tradingJournal", name: t.dashboard.tradingJournal, path: `/${currentLocale}/dashboard/trading-journal`, icon: <TrendingUp size={22} /> },
    { id: "wallet", name: t.dashboard.wallet, path: `/${currentLocale}/dashboard/wallet`, icon: <Wallet size={22} /> },
    { id: "achievements", name: t.dashboard.achievements, path: `/${currentLocale}/dashboard/achievements`, icon: <Trophy size={22} /> },
    { id: "aiAssistant", name: t.dashboard.aiAssistant, path: "#", icon: <Bot size={22} />, disabled: true },
    { id: "support", name: t.dashboard.support, path: `/${currentLocale}/support`, icon: <Headset size={22} /> },
    { id: "settings", name: t.dashboard.settings, path: `/${currentLocale}/dashboard/settings`, icon: <Settings size={22} /> },
  ];
  ...
  const getMenuColor = (id: string) => {
    switch (id) {
      ...
      case "profile": return "from-emerald-500/20 to-emerald-500/5 text-emerald-400 border-emerald-500/30";
      case "announcements": return "from-orange-500/20 to-orange-500/5 text-orange-400 border-orange-500/30";
      case "liveCampus": return "from-red-500/20 to-red-500/5 text-red-400 border-red-500/30";
      ...
      case "wallet": return "from-yellow-500/20 to-yellow-500/5 text-yellow-400 border-yellow-500/30";
  ...
  if (!isReady) {
    return (
      <div dir={t.dir} className="min-h-screen flex flex-col items-center justify-center bg-[#020202] text-white">
        ...
        <span className="text-[#C2185B] font-bold uppercase tracking-widest text-xs animate-pulse">{t.dashboard.initializing}</span>
      </div>
    );
  }

  return (
    <div dir={t.dir} className="flex h-screen bg-[#030305] text-white font-sans overflow-hidden relative selection:bg-[#C2185B] selection:text-white">
      ...
      <aside className={`hidden lg:flex w-[295px] bg-[#060609]/90 backdrop-blur-2xl ${isRtl ? 'border-l border-r-0' : 'border-r border-l-0'} border-white/[0.06] flex-col relative z-20 shrink-0 p-4 shadow-[15px_0_40px_rgba(0,0,0,0.9)]`}>
        ...
        <div className="mb-5 p-4 rounded-2xl bg-gradient-to-br from-[#0c0c14] to-[#07070a] shadow-[inset_0_2px_6px_rgba(0,0,0,0.9),0_6px_20px_rgba(0,0,0,0.5)] border border-white/5 shrink-0 relative group">
          ...
          <Link href={`/${currentLocale}/dashboard`} className="flex items-center gap-3.5 relative z-10">
            ...
            <div>
              ...
              <p className="text-[9px] text-neutral-400 font-bold uppercase tracking-[0.2em] mt-0.5">{t.dashboard.welcomeBack}</p>
            </div>
          </Link>
        </div>
        ...
        <nav className={`flex-1 overflow-y-auto space-y-2 custom-scrollbar ${isRtl ? 'pl-1 pr-0' : 'pr-1 pl-0'}`}>
          {menuItems.map((item) => {
            if (item.disabled) {
              return (
                <div key={item.id} className="flex items-center gap-4 px-4 py-3 rounded-2xl font-bold text-xs transition-all duration-300 relative group overflow-hidden text-neutral-500 opacity-60 cursor-not-allowed bg-[#08080c]/50 border border-white/[0.02]">
                  <span className="text-neutral-500">{item.icon}</span>
                  <span className="tracking-wide">{item.name}</span>
                  <span className={`absolute ${isRtl ? 'left-4' : 'right-4'} text-[8px] font-black uppercase tracking-widest bg-yellow-500/10 text-yellow-500 px-2 py-0.5 rounded border border-yellow-500/20 shadow-[0_0_10px_rgba(234,179,8,0.2)]`}>{t.dashboard.soon}</span>
                </div>
              );
            }
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-4 px-4 py-3 rounded-2xl font-bold text-xs transition-all duration-300 relative group overflow-hidden ${isActive
                  ? `bg-gradient-to-r from-[#1c1a12] to-[#0a0a0e] text-white shadow-[inset_0_2px_5px_rgba(0,0,0,0.9),0_0_20px_rgba(194,24,91,0.15)] border border-[#C2185B]/30 ${isRtl ? '-translate-x-1' : 'translate-x-1'}`
              ...
              >
                {isActive && <div className={`absolute ${isRtl ? 'right-0 rounded-l-full' : 'left-0 rounded-r-full'} top-1/2 -translate-y-1/2 w-1 h-6 bg-[#C2185B] shadow-[0_0_10px_#C2185B]`}></div>}
        ...
        <div className="mt-4 pt-4 border-t border-white/[0.06] space-y-3 shrink-0">
          ...
          <button onClick={handleLogout} className="flex items-center justify-center gap-2 px-4 py-3 text-neutral-400 hover:text-red-400 bg-[#08080c] hover:bg-red-500/10 rounded-2xl text-xs font-bold transition-all w-full border border-white/[0.04] cursor-pointer">
            <LogOut size={14} className={isRtl ? "rotate-180" : ""} /> {t.dashboard.signOut}
          </button>
        </div>
      </aside>
      ...
      {!isFullScreenRoute && (
        <div className="lg:hidden fixed bottom-4 left-4 right-4 h-[70px] bg-[#09090e]/95 backdrop-blur-3xl border border-white/10 z-50 px-2 rounded-[2.2rem] flex justify-between items-center shadow-[0_20px_50px_rgba(0,0,0,0.9)]">
          <Link href={`/${currentLocale}/dashboard`} className="relative flex flex-col items-center justify-center w-[25%] h-full group">
            ...
            <span className="text-[8px] font-bold uppercase mt-1">{t.dashboard.overview}</span>
          </Link>
          <Link href={`/${currentLocale}/feed`} className="relative flex flex-col items-center justify-center w-[25%] h-full group">
            ...
            <span className="text-[8px] font-bold uppercase mt-1">{t.dashboard.feed}</span>
          </Link>
          <Link href={`/${currentLocale}/dashboard/live-classes`} className="relative flex flex-col items-center justify-center w-[25%] h-full group">
            ...
            <span className="text-[8px] font-bold uppercase mt-1">{t.dashboard.live}</span>
          </Link>
          <button onClick={() => setIsMobileMenuOpen(true)} className="relative flex flex-col items-center justify-center w-[25%] h-full group">
            ...
            <span className="text-[8px] font-bold uppercase mt-1">{t.dashboard.menu}</span>
          </button>
        </div>
      )}
      ...
      {isMobileMenuOpen && (
        <div dir={t.dir} className="fixed inset-0 bg-[#030305]/95 backdrop-blur-3xl z-[100] flex flex-col lg:hidden">
          <div className="h-16 px-5 border-b border-white/[0.06] flex justify-between items-center bg-black/50 shrink-0">
            <span className="font-black text-white uppercase tracking-widest text-xs">{t.dashboard.toolsFeatures}</span>
          ...
          <div className="flex-1 overflow-y-auto p-5 custom-scrollbar space-y-6">
            <div className="grid grid-cols-2 gap-3.5">
              {menuItems.map((item) => {
                const isActive = item.id === "profile" ? pathname.includes(`/${currentLocale}/feed/profile`) : item.path !== "#" && pathname.startsWith(item.path);
                const colorClasses = getMenuColor(item.id);
                if (item.disabled) {
                  return (
                    <div key={item.id} className={`flex flex-col items-center justify-center gap-3 p-5 rounded-[1.8rem] font-bold border opacity-50 cursor-not-allowed ${colorClasses}`}>
                ...
                return (
                  <Link key={item.id} href={item.path} onClick={() => setIsMobileMenuOpen(false)} className={`flex flex-col items-center justify-center gap-2 p-5 rounded-[1.8rem] font-bold border bg-gradient-to-br ${colorClasses} ${isActive ? "ring-2 ring-white/20" : ""}`}>
                    <div>{item.icon}</div>
                    <span className="text-[10px] tracking-widest uppercase text-center">{item.name}</span>
                  </Link>
                );
            ...
            <div className="pt-2 pb-6">
              <button onClick={handleLogout} className="flex items-center justify-center gap-2 px-4 py-3.5 text-neutral-300 hover:text-red-400 bg-[#08080c] hover:bg-red-500/10 rounded-2xl text-xs font-bold transition-all w-full border border-white/[0.06] shadow-md cursor-pointer">
                <LogOut size={16} className={isRtl ? "rotate-180" : ""} /> {t.dashboard.signOut}
              </button>
            </div>
          </div>
        </div>
