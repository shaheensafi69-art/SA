...
export default function AIAssistantPage() {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      const aiResponseCall = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ prompt: userText, userId: userId }),
      });

      const resData = await aiResponseCall.json().catch(() => ({}));

      if (!aiResponseCall.ok) {
        throw new Error(resData.message || `Server responded with status ${aiResponseCall.status}`);
      }
      
      const realAIResponse = resData.message;
      ...
      setMessages(prev => [
        ...prev,
        { id: `error-${Date.now()}`, role: "ai", content: `⚠️ ${error.message}`, created_at: new Date().toISOString() }
      ]);
    } finally {
      setIsTyping(false);
    }
  ...
  return (
    // 🔥 کلاس‌های هوشمند: موبایل کاملاً فول‌اسکرین (fixed z-[100])، دسکتاپ فقط پر کردن فضای خالی کنار سایدبار (lg:relative) 🔥
    <div className="fixed inset-0 z-[100] lg:relative lg:inset-auto lg:z-10 lg:w-full lg:h-full flex flex-col bg-[#030305] font-sans overflow-hidden select-none">
        ...
        <div className="max-w-4xl mx-auto flex flex-col gap-4">
                ...
                <div key={msg.id} className={`flex w-full ${isMe ? "justify-end" : "justify-start"} items-end gap-2 animate-[fadeInUp_0.2s_ease-out]`}>
                  ...
                  <div className="flex flex-col max-w-[85%] sm:max-w-[70%]">
                    <div className={`relative px-4 py-3 shadow-2xl transition-all duration-300 text-[13px] sm:text-sm ${
                      isMe ? "bg-gradient-to-br from-indigo-600 to-blue-600 text-white rounded-2xl rounded-br-none border border-white/10 shadow-[0_4px_20px_rgba(79,70,229,0.15)] font-medium" : "bg-[#14141c]/90 backdrop-blur-xl border border-white/5 text-neutral-200 rounded-2xl rounded-tl-none leading-relaxed"
                    }`}>
          ...
          {isTyping && (
            <div className="flex justify-start items-end gap-2 animate-[fadeIn_0.2s_ease-out]">
              ...
              <div className="px-4 py-3.5 bg-[#14141c]/90 border border-white/5 rounded-2xl rounded-tl-none flex items-center gap-1.5">
        ...
        <div className="max-w-4xl mx-auto flex flex-col gap-2">
          ...
          {showEmojiPanel && (
            <div className="p-3 bg-black/40 border border-white/5 rounded-2xl flex flex-wrap justify-center gap-2 text-xl animate-[fadeIn_0.2s_ease-out]">
              {["🔥", "🚀", "💻", "📈", "📊", "🎯", "💰", "💎", "💡", "🧠", "👍", "🙌", "🎉", "👑"].map(emoji => (
                <span key={emoji} onClick={() => setInputValue(prev => prev + emoji)} className="cursor-pointer hover:scale-125 transition-transform p-1 select-none active:scale-90">{emoji}</span>
              ))}
            </div>
          )}
        </div>
