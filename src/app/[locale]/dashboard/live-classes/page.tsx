...
export default function LiveClassesDashboard() {
      ...
      if (data) {
        const formatted = data.map((cls: any) => {
          ...
          return {
            ...
            is_paid: studentRelation?.is_paid ?? false,
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-white p-6 sm:p-10 relative overflow-hidden" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="max-w-7xl mx-auto space-y-10 relative z-10 animate-[fadeIn_0.5s_ease-out]">
        ...
        {/* Header */}
        <header className="rounded-[2rem] border border-white/5 bg-neutral-950/40 p-8 backdrop-blur-3xl shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 text-start">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.35em] text-indigo-400 mb-2">{t.liveClasses.headquarters}</p>
            <h1 className="text-3xl sm:text-4xl font-black tracking-tight">{t.liveClasses.title}</h1>
            <p className="text-neutral-400 text-sm mt-2 max-w-xl leading-relaxed">{t.liveClasses.desc}</p>
          </div>
          <div className="flex bg-white/5 border border-white/10 p-4 rounded-2xl items-center gap-4">
             ...
             <div>
                <p className="text-xs text-neutral-500 font-bold uppercase tracking-widest">{t.liveClasses.enrolledChannels}</p>
        ...
        {isLoading ? (
          ...
          <div className="space-y-12">
            ...
            <div>
              <h2 className="text-xl font-extrabold flex items-center gap-2.5 mb-6 text-start">
                <span className="flex h-3 w-3 relative">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-3 w-3 bg-red-500"></span>
                </span>
                {t.liveClasses.liveTransmissions} ({liveSessions.length})
              </h2>
              ...
              {liveSessions.length === 0 ? (
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {liveSessions.map((room) => (
                    <div 
                      key={room.id} 
                      className={`relative rounded-3xl border p-6 flex flex-col h-full group transition-all duration-300 text-start ${
                        room.is_paid 
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        {room.is_paid ? (
                          <span className="bg-red-500 text-black text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md shadow-md animate-pulse">{t.liveClasses.liveNow}</span>
                        ) : (
                          <span className="bg-amber-500/10 border border-amber-500/30 text-amber-400 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md flex items-center gap-1.5">
                            <LockKeyhole size={12} /> {t.liveClasses.pendingPayment}
                          </span>
                        )}
                      </div>
                      ...
                      <p className="text-xs text-neutral-400 mt-2 font-medium flex items-center gap-1.5">
                        <User size={14} className="text-neutral-600" /> {t.liveClasses.instructor}: {room.teacher ? `${room.teacher.first_name} ${room.teacher.last_name}` : t.liveClasses.facultyMember}
                      </p>
                        ...
                        {room.is_paid ? (
                          <>
                            {room.meeting_link ? (
                              ...
                              >
                                <Video size={14} /> {t.liveClasses.joinTeams} <ExternalLink size={12} className="opacity-50" />
                              </a>
                            ) : (
                            ...
                            {room.signal_group_link ? (
                              ...
                              >
                                <MessageSquare size={14} className="text-indigo-400" /> {t.liveClasses.signalOps} <ExternalLink size={12} className="opacity-30" />
                              </a>
                            ) : (
                              <span className="w-full text-center text-[10px] text-neutral-600 font-bold uppercase tracking-widest py-2">
                                {t.liveClasses.signalPending}
                              </span>
                            )}
                          </>
                        ) : (
                          <div className="bg-black/60 border border-white/5 rounded-2xl p-4 text-center space-y-2">
                            <p className="text-xs font-bold text-neutral-400">{t.liveClasses.paymentLockedNotice}</p>
                            <span className="text-[10px] font-black uppercase tracking-widest text-amber-500/80 block">{t.liveClasses.contactSupport}</span>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div>
              <h2 className="text-xl font-extrabold text-neutral-300 mb-6 flex items-center gap-2 text-start">
                <span>📚</span> {t.liveClasses.scheduledChannels} ({generalClasses.length})
              </h2>

              {generalClasses.length === 0 ? (
                <div className="rounded-3xl border border-white/5 p-10 text-center text-neutral-600 text-sm">
                  {t.liveClasses.noScheduled}
                </div>
              ) : (
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {generalClasses.map((room) => (
                    <div 
                      key={room.id} 
                      className={`rounded-3xl border p-6 transition-all duration-300 flex flex-col h-full group text-start ${
                        room.is_paid 
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        {room.is_paid ? (
                          <span className="bg-white/5 border border-white/10 text-neutral-400 text-[9px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-md">{t.liveClasses.standby}</span>
                        ) : (
                          <span className="bg-amber-500/10 border border-amber-500/30 text-amber-400 text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md flex items-center gap-1.5">
                            <LockKeyhole size={12} /> {t.liveClasses.locked}
                          </span>
                        )}
                      </div>
                        ...
                        {room.is_paid ? (
                          <>
                            {room.signal_group_link ? (
                              ...
                              >
                                <MessageSquare size={14} /> {t.liveClasses.openSignal}
                              </a>
                            ) : (
                              <button disabled className="w-full py-3.5 bg-white/[0.02] border border-white/5 text-neutral-600 rounded-xl text-xs font-black uppercase tracking-widest cursor-not-allowed">
                                {t.liveClasses.channelLocked}
                              </button>
                            )}
                          </>
                        ) : (
                          <div className="bg-black/60 border border-white/5 rounded-2xl p-4 text-center">
                            <p className="text-xs font-bold text-neutral-500">{t.liveClasses.awaitingValidation}</p>
                          </div>
                        )}
                      </div>
                    </div>
