  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-neutral-500 text-xs font-black uppercase tracking-widest animate-pulse">{t.adminPages.auditingFinancialRecords}</p>
      </div>
    );
  }
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto space-y-6 sm:space-y-8 animate-[fadeIn_0.4s_ease-out]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          <div className="bg-[#0a0a0f]/80 p-5 sm:p-6 rounded-[2rem] border border-white/5 shadow-xl backdrop-blur-xl">
            ...
            <p className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-1">{t.adminPages.grossRevenueInflows}</p>
          ...
          <div className="bg-[#0a0a0f]/80 p-5 sm:p-6 rounded-[2rem] border border-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.05)] backdrop-blur-xl relative overflow-hidden">
            ...
            <p className="relative z-10 text-[10px] font-black text-amber-500/70 uppercase tracking-widest mb-1">{t.adminPages.facultyLiabilityUnpaid}</p>
          ...
          <div className="bg-[#0a0a0f]/80 p-5 sm:p-6 rounded-[2rem] border border-white/5 shadow-xl backdrop-blur-xl">
            ...
            <p className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-1">{t.adminPages.totalDistributedPayouts}</p>
          ...
          <div className="bg-[#0a0a0f]/80 p-5 sm:p-6 rounded-[2rem] border-l-2 border-emerald-500/50 shadow-[inset_10px_0_20px_rgba(16,185,129,0.05)] backdrop-blur-xl">
            ...
            <p className="text-[10px] font-black text-emerald-500/70 uppercase tracking-widest mb-1">{t.adminPages.estimatedNetProfit}</p>
        ...
        {activeTab === "faculty" && (
          <div className="bg-[#0a0a0f]/80 border border-white/5 rounded-[2.5rem] p-6 sm:p-8 backdrop-blur-3xl shadow-2xl animate-[fadeIn_0.3s_ease-out]">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              <h3 className="text-xl font-black text-white flex items-center gap-2"><Wallet size={20} className="text-amber-400"/> {t.adminPages.pendingFacultyPayouts}</h3>
              <div className="bg-black/40 p-2 rounded-2xl border border-white/5 flex items-center gap-2 w-full sm:max-w-xs shadow-inner">
                ...
                <input 
                  type="text" placeholder={t.adminPages.findInstructorPlaceholder}
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
              {filteredTeachers.length === 0 ? (
                <div className="text-center py-12 text-neutral-500">{t.adminPages.noInstructorsFound}</div>
              ) : (
                  ...
                  <div key={teacher.id} className="w-full text-left rounded-2xl border border-white/5 bg-black/40 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-white/10 transition-colors">
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-6 sm:w-auto w-full border-t border-white/5 sm:border-0 pt-4 sm:pt-0">
                      <div className="text-left sm:text-right">
                        <p className="text-[9px] font-black uppercase tracking-widest text-neutral-500">{t.adminPages.unpaidShare}</p>
        ...
        {activeTab === "inflows" && (
          <div className="bg-[#0a0a0f]/80 border border-white/5 rounded-[2.5rem] p-6 sm:p-8 backdrop-blur-3xl shadow-2xl animate-[fadeIn_0.3s_ease-out]">
            <h3 className="text-xl font-black text-white flex items-center gap-2 mb-6 border-b border-white/5 pb-4"><TrendingUp size={20} className="text-emerald-400"/> {t.adminPages.studentPaymentsLedger}</h3>
            
            <div className="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
              {studentPayments.length === 0 ? (
                <div className="text-center py-12 text-neutral-500">{t.adminPages.noPaymentRecords}</div>
              ) : (
          ...
          <div className="bg-[#0a0a0f]/80 border border-white/5 rounded-[2.5rem] p-6 sm:p-8 backdrop-blur-3xl shadow-2xl animate-[fadeIn_0.3s_ease-out]">
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
              {payoutHistory.length === 0 ? (
                <div className="text-center py-12 text-neutral-500">{t.adminPages.noPayoutsProcessed}</div>
              ) : (
        ...
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-[slideInUp_0.2s_ease-out] sm:animate-[fadeIn_0.2s_ease-out]">
          ...
          <div className="relative w-full max-w-md bg-[#0a0a0f] border-t sm:border border-white/10 rounded-t-[2rem] sm:rounded-[2.5rem] shadow-[0_-20px_80px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col">
            ...
            <div className="p-6 border-b border-white/5 bg-neutral-900/40 shrink-0 flex justify-between items-center">
              <div>
                <h2 className="text-base font-black text-white">{t.adminPages.processFacultyPayout}</h2>
            ...
            <div className="p-6 sm:p-8 bg-[#050508] space-y-6">
              ...
              <form onSubmit={handleProcessPayout} className="space-y-6">
                <div className="bg-black/40 border border-white/5 rounded-2xl p-5 shadow-inner text-center">
                  <p className="text-[10px] text-neutral-500 font-black uppercase tracking-widest mb-1">{t.adminPages.unpaidBalance}</p>
                  <p className="text-3xl font-black text-amber-400">${selectedTeacher.wallet_balance.toFixed(2)}</p>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-neutral-500 uppercase tracking-widest block ml-1">{t.adminPages.transferAmount}</label>
                  ...
                  <p className="text-[9px] text-neutral-500 ml-1">{t.adminPages.payoutDeductFundsDesc}</p>
                </div>
